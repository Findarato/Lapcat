...
class Materials{
	...
	//
	//
	// Function - Change Tag Name
	function F_ChangeTagName($v_TagName=''){$this->V_TagName=$v_TagName;}
	//
	// Function - Change Search
	function F_ChangeSearch($v_Key='',$v_Search=0){switch($v_Key){case '':break;default:if(array_key_exists($v_Key,$this->A_Search)){$this->A_Search[$v_Key]=$v_Search;}break;}}
	//
			...
			switch($v_Key){
				...
				case 'change-search':
					switch($v_Search){
						...
						case 1:case 2:case 3:case 4:case 5:case 23:case 24:case 32:case 50:case 75:$a_Description['search']=' '.$this->A_MaterialTypes[$v_Search];break;
					}break;
				case 'change-sort':
					switch($v_Search){
	...
	function F_ConstructSearch($v_UserID=0){
		...
		foreach($this->A_Search as $v_Key=>$v_Search){
			switch($v_Key){
				...
				case 'change-search':
					switch($v_Search){
						case 0:default:$a_SQL['material-type']='>0';$a_SQL['where-A']=' WHERE htbm.tag_ID IN(1,2,3,4,5,23,24,32,50,75)';$a_Description[1]=' materials';break;
						case 1:case 2:case 3:case 4:case 5:case 23:case 24:case 32:case 50:case 75:
				...
				case 'change-tag':
					switch($v_Search){
						case 0:$a_SQL['where-C']='';$a_SQL['SQL-E']=')';break;
						default:$a_SQL['where-C']=' AND htbma.tag_ID='.$v_Search;$a_SQL['SQL-E']=') LEFT JOIN hex_tags_by_material AS htbma ON (htbma.ID=htbm.ID)';break;
					}break;
			}
		}
	...
	function F_PerformSearch($v_UserID=0){
		$v_SQL=$this->F_ConstructSearch($v_UserID);
		print_r($v_SQL);die();
		$v_DC=db::getInstance();
		$v_DC->Query($v_SQL);
		if($v_DC->Count_res()>0){
	...
	function F_LoadPreviousSearch($v_UserID=0){$this->A_Parameters=$this->A_PreviousSearch['parameters'];$this->A_Search=$this->A_PreviousSearch['search'];}
	//
	// Function - Save Previous Search
	function F_SavePreviousSearch($v_UserID=0){$this->A_PreviousSearch=array('parameters'=>$this->A_Parameters,'search'=>$this->A_Search);}
