...
class Online{
	...
	function f_IncreaseAbility($v_TeamName,$v_MemberID,$v_PerformanceName,$v_EffectName,$v_Amount=1){
	...
	/* Function - Increase Ability All */
	function f_IncreaseAbilityAll($v_TeamName,$v_MemberID,$v_PerformanceName,$v_Amount=1){
		$this->f_IncreaseAbility($v_TeamName,$v_MemberID,$v_PerformanceName,'effect',$v_Amount);
		$this->f_IncreaseAbility($v_TeamName,$v_MemberID,$v_PerformanceName,'critical',$v_Amount);
		$this->f_IncreaseAbility($v_TeamName,$v_MemberID,$v_PerformanceName,'damage',$v_Amount);
	}
	/* Function - Increase Ability Boost */
	function f_IncreaseAbilityBoost($v_TeamName,$v_MemberID,$v_PerformanceName,$v_EffectName,$v_Amount=1){$this->A_Teams[$v_TeamName][$v_MemberID][$v_PerformanceName][$v_EffectName]['boost']+=$v_Amount;}
	/* Function - Increase Ability Chances */
	function f_IncreaseAbilityChances($v_TeamName,$v_MemberID,$v_PerformanceName,$v_EffectName,$v_Amount=1){$this->A_Teams[$v_TeamName][$v_MemberID][$v_PerformanceName][$v_EffectName]['chances']+=$v_Amount;}
	/* Function - Increase Skill */
	function f_IncreaseSkill($v_TeamName,$v_MemberID,$v_SkillName,$v_Amount=1){$this->A_Teams[$v_TeamName][$v_MemberID]['skills'][$v_SkillName]+=$v_Amount;}




	/* Function - Add Member To Team */
	function f_AddCharacterToTeam($v_TeamName,$a_Parameters){
	...
	function f_Continue(){
		...
		if($this->v_FailSafeCounter>$this->A_Quest['maximum-turns-fail-safe']||$this->A_TeamData['party-has-won']||$this->A_TeamData['quest-has-won']){
			if($this->A_TeamData['quest-has-won']){$this->A_Statistics['quest']['success']=true;}else{if($this->A_TeamData['party-has-won']){$this->A_Statistics['party']['success']=true;}}
			/* Statistics */
			$this->A_Statistics['total-turns']=$this->A_TeamData['turn-counter']; /* Statistics */
			$this->A_Statistics['total-time']=$this->f_GetTime(); /* Statistics */
			$this->f_CalculateRate('party'); /* Statistics */
			$this->f_CalculateRate('quest'); /* Statistics */
	...
	/* Statistics */
	function f_CalculateRatePerHour($v_TeamName,$v_MemberID){$this->A_Teams[$v_TeamName][$v_MemberID]['hire-rate-per-hour']=12*$this->A_Teams[$v_TeamName][$v_MemberID]['hire-rate-per-turn'];}
	...
	function f_ApplyMethod($v_TeamName,$v_MemberID,$v_PerformanceName,$v_EffectName){
		switch($v_EffectName){
			...
			case 'Afraid':
				...
				break;
			case 'Death':
				$a_Parameters=array('','Intellect','','Health');
				break;
			case '':default:
				$a_Parameters=array('','','','');
				break;
	...
	function f_NewApplyComponent($v_TeamName,$v_MemberID,$v_PerformanceName,$v_Component){
		...
		switch($v_Component){
			...
			case 'Ring Image':
				...
				$this->A_Teams[$v_TeamName][$v_MemberID]['defense']['images']['finger']=$a_Images[$v_Component];
				break;
			...
			case 'Gauntlets Image':case 'Gold Gauntlets Image':
				...
				$this->A_Teams[$v_TeamName][$v_MemberID]['defense']['images']['hands']=$a_Images[$v_Component];
				break;
			/* Defense - Head Images */
			case 'Helm Image':
				$a_Images=array(
				...
				$this->A_Teams[$v_TeamName][$v_MemberID]['defense']['images']['head']=$a_Images[$v_Component];
				break;
			/* Defense - Legs Images */
			case 'Leggings Image':
				$a_Images=array(
				...
				$this->A_Teams[$v_TeamName][$v_MemberID]['defense']['images']['legs']=$a_Images[$v_Component];
				break;
			...
			case 'Thunder-Blast Effect':
				$this->f_AddEffect($v_TeamName,$v_MemberID,$v_PerformanceName,'effect','Deaf And Down');
				$this->A_Teams[$v_TeamName][$v_MemberID]['hire-value']+=240;
				break;
			case 'Lure Effect':
			...
			case 'Decay Effect':
				...
				break;
			case 'Death Effect':
				$this->f_AddEffect($v_TeamName,$v_MemberID,$v_PerformanceName,'effect','Death');
			...
			case 'Knock-Down Effect':
				...
				break;
			case 'Stun Effect':
				$this->f_AddEffect($v_TeamName,$v_MemberID,$v_PerformanceName,'effect','Stunned');
				$this->A_Teams[$v_TeamName][$v_MemberID]['hire-value']+=120;
				break;
	...
	function f_ApplyConditionsToMember($v_TeamName,$v_MemberID){
		...
		foreach($this->A_Teams[$v_TeamName][$v_MemberID]['traits'] as $v_Key=>$v_TraitName){
			...
			switch($v_TraitName){
				/* Race */
				case 'Barbarian':$this->f_NewApplyComponents($v_TeamName,$v_MemberID,'attack',array('Barbarian Image','Alive','Brutish','Flesh','Good Health','Fearless','Mobile','Large-Size','Unsusceptible'));break;
				case 'Bat':$this->f_NewApplyComponents($v_TeamName,$v_MemberID,'attack',array('Bat Image','Alive','Flying','Black Fur','Great Understanding','Mobile','Perceptive','Quick','Tiny-Size'));break;
				case 'Bear':$this->f_NewApplyComponents($v_TeamName,$v_MemberID,'attack',array('Bear Image','Alive','Fur','Great Detection','Brutish','Good Health','Hide','Large-Size','Mobile','Perceptive'));break;
				...
				case 'Dwarf':$this->f_NewApplyComponents($v_TeamName,$v_MemberID,'attack',array('Dwarf Image','Alive','Flesh','Brutish','Good Health','Mobile','Perceptive','Small-Size','Good Strength'));break;
				...
				case 'Ghost':$this->f_NewApplyComponents($v_TeamName,$v_MemberID,'attack',array('Ghost Image','Flying','Good Stealth','Half-Intelligent','Ethereal','Spirit','Medium-Size','Mobile','Poison Immunity','Unsusceptible'));break;
				case 'Giant':$this->f_NewApplyComponents($v_TeamName,$v_MemberID,'attack',array('Giant Image','Alive','Dumb','Brutish','Flesh','Fearless','Mobile','Giant-Size','Slow','Greatest Health','Poor Eyesight','Unsusceptible','Thick Hide'));break;
				...
				case 'Halfling':$this->f_NewApplyComponents($v_TeamName,$v_MemberID,'attack',array('Halfling Image','Alive','Flesh','Good Detection','Half-Intelligent','Quick','Small-Size','Mobile','Perceptive'));break;
				...
				case 'Lich':$this->f_NewApplyComponents($v_TeamName,$v_MemberID,'attack',array('Lich Image','Fearless','Flesh','Intelligent','Poor Aiming','Medium-Size','Mobile','Poor Eyesight','Slow','Natural Regeneration','Poison Immunity','Undead','Unsusceptible'));break;
				case 'Mummy':$this->f_NewApplyComponents($v_TeamName,$v_MemberID,'attack',array('Mummy Image','Fearless','Half-Intelligent','Poor Aiming','Medium-Size','Mobile','Poor Eyesight','Slow','Natural Regeneration','Poison Immunity','Undead','Unsusceptible'));break;
				case 'Ogre':$this->f_NewApplyComponents($v_TeamName,$v_MemberID,'attack',array('Ogre Image','Alive','Half-Dumb','Brutish','Flesh','Mobile','Large-Size','Very Slow','Great Health','Poor Eyesight','Unsusceptible','Thick Hide'));break;
				...
				case 'Spider':$this->f_NewApplyComponents($v_TeamName,$v_MemberID,'attack',array('Spider Image','Alive','Fur','Mobile','Multiple Eyes','Multiple Legs','Perceptive','Scurry','Tiny-Size'));break;
				...
				case 'Wolf':$this->f_NewApplyComponents($v_TeamName,$v_MemberID,'attack',array('Wolf Image','Alive','Fearless','Fur','Good Perception','Great Detection','Hide','Small-Size','Mobile','Perceptive','Phonemic','Quick','Stalker'));break;
				case 'Wraith':$this->f_NewApplyComponents($v_TeamName,$v_MemberID,'attack',array('Wraith Image','Flying','Great Stealth','Intelligent','Ethereal','Spirit','Medium-Size','Mobile','Poison Immunity','Unsusceptible'));break;
				...
				case 'Zombie':$this->f_NewApplyComponents($v_TeamName,$v_MemberID,'attack',array('Zombie Image','Almost Alive','Undead','Fearless','Dumb','Flesh','Horrible Aiming','Medium-Size','Mobile','Poor Eyesight','Slow','Natural Regeneration','Unsusceptible'));break;
		...
		foreach($this->A_Teams[$v_TeamName][$v_MemberID]['items'] as $v_ItemSlot=>$v_ItemName){
			...
			/* Plate Armor (Heavy - Very Heavy) */
				...
				case 'Knight Of Zeal Platemail':$this->f_NewApplyComponents($v_TeamName,$v_MemberID,'attack',array('Gold Plate Mail Image','Cloth Of The King','Cotton Padding','Tough Leather Hide','Spider-Silk Inserts','Metal Plates','Very Protective','Reflective','Heavy','Well-Built'));break;
				case 'Knight Of Zeal Gauntlets':$this->f_NewApplyComponents($v_TeamName,$v_MemberID,'attack',array('Gold Gauntlets Image','Cloth Of The King','Spider-Silk Inserts','Leather Hide','Metal Plates','Protective','Very Reflective','Well-Built'));break;
				...
				/* Steel Armor */
				case 'Steel Helm':$this->f_NewApplyComponents($v_TeamName,$v_MemberID,'attack',array('Helm Image','Close Helm','Cloth','Cotton Padding','Half-Leather Hide','Metal Plates','Protective','Reflective','Heavy'));break;
				...
				/* Iron Armor */
				case 'Iron Helm':$this->f_NewApplyComponents($v_TeamName,$v_MemberID,'attack',array('Helm Image','Close Helm','Cotton Padding','Half-Leather Hide','Metal Plates','Protective','Reflective','Heavy'));break;
			...
			/* Cloth Armor (Very Light - Light) */
				...
				/* Decaying Armor */
				case 'Decaying Hat':$this->f_NewApplyComponents($v_TeamName,$v_MemberID,'attack',array('Black Cloth','Magical','Rune-Lined','Very Light'));break;
				case 'Decaying Robe':$this->f_NewApplyComponents($v_TeamName,$v_MemberID,'attack',array('Black Cloth','Magical','Robes Image','Rune-Lined','Protective','Very Light'));break;
				case 'Decaying Pantaloons':$this->f_NewApplyComponents($v_TeamName,$v_MemberID,'attack',array('Black Cloth','Magical','Rune-Lined','Protective','Very Light'));break;

				/* Soul-Trapped Armor */
				case 'Soul-Trapped Handwraps':$this->f_NewApplyComponents($v_TeamName,$v_MemberID,'attack',array('Black Cloth','Magical','Rune-Covered','Very Light'));break;
				case 'Soul-Trapped Sandals':$this->f_NewApplyComponents($v_TeamName,$v_MemberID,'attack',array('Black Cloth','Magical','Rune-Covered','Very Light'));break;
				...
				/* W a n d s */
				case 'Wand Of Fire Bolt':$this->f_NewApplyComponents($v_TeamName,$v_MemberID,'attack',array('Fire Wand Image','Deflection','Heat Damage','Short-To-Distant Range','Deadly','Main','Protective','Very Light','Wooden Handle','Slow Recovery','Arcane-Powered','Lesser Level','Novice Power'));break;
				case 'Wand Of Magic Missile':$this->f_NewApplyComponents($v_TeamName,$v_MemberID,'attack',array('Wand Image','Deflection','Piercing Damage','Near-By Range','Hostile','Main','Protective','Very Light','Wooden Handle','Slow Recovery','Arcane-Powered','Lesser Level','Master Power','Must Block'));break;
				...
				case 'Poison-Tipped Dagger':$this->f_NewApplyComponents($v_TeamName,$v_MemberID,'attack',array('Poison Dagger Image','Deflection','Close Range','Deadly','Dual','Poison Damage','Short Blade','Thrusting Blade','Very Light'));break;
				...
				case 'Spiked Knuckles':$this->f_NewApplyComponents($v_TeamName,$v_MemberID,'attack',array('Fangs Image','Deflection','Close Range','Deadly','Main','Small Metal Spikes','Slashing Damage','Very Light'));break;
				...
				case 'Staff Of Voices':$this->f_NewApplyComponents($v_TeamName,$v_MemberID,'attack',array('Staff Image','Challenge','Stun Effect','Close Range','Hostile','Long Wooden Rod','Protective','Two-Handed','Powered-By-Will','Must Understand','Amateur Level','Slow Recovery'));break;
				case 'Oak Staff':$this->f_NewApplyComponents($v_TeamName,$v_MemberID,'attack',array('Staff Image','Challenge','Trip Effect','Close Range','Hostile','Long Wooden Rod','Protective','Two-Handed'));break;
				case 'Cedar Staff':$this->f_NewApplyComponents($v_TeamName,$v_MemberID,'attack',array('Staff Image','Challenge','Trip Effect','Close Range','Hostile','Long Wooden Rod','Two-Handed'));break;
				...
				case '':
					if($v_ItemSlot=='left-hand'||$v_ItemSlot=='right-hand'){
						...
						if($v_ItemSlot=='left-hand'){
							$this->f_NewApplyComponents($v_TeamName,$v_MemberID,'attack',array('Empty Image','Deflection','Blunt Damage','Close Range','Dual','Deadly','Amateur Power','Very Light'));
						}else{
							$this->f_NewApplyComponents($v_TeamName,$v_MemberID,'attack',array('Empty Image','Amateur Power','Very Light'));
						}
						break;
		...
		foreach($this->A_Teams[$v_TeamName][$v_MemberID]['training'] as $v_Key=>$v_TrainingName){
			...
			switch($v_TrainingName){
				...
				case 'Decaying Flesh': /* When this character dies, a Skeleton Slasher enters the game. */
					$this->A_Teams[$v_TeamName][$v_MemberID]['on-death-effect']='Decaying Flesh';
					$this->A_Teams[$v_TeamName][$v_MemberID]['hire-value']+=960;
					break;

				case 'Mummy Wrappings': /* When this character dies, a Skeleton Penetrator enters the game. */
					$this->A_Teams[$v_TeamName][$v_MemberID]['on-death-effect']='Mummy Wrappings';
					$this->A_Teams[$v_TeamName][$v_MemberID]['hire-value']+=960;
					break;
				...
				case 'Novice Acrobat':$this->A_Teams[$v_TeamName][$v_MemberID]['skills']['Balance']++;break;
				case 'Skeleton Balance':case 'Adept Acrobat':$this->A_Teams[$v_TeamName][$v_MemberID]['skills']['Balance']+=2;break;
				case 'Master Acrobat':$this->A_Teams[$v_TeamName][$v_MemberID]['skills']['Balance']+=3;break;
				...
				case 'Zombie Block':case 'Adept Blocker':$this->A_Teams[$v_TeamName][$v_MemberID]['skills']['Block']+=2;break;
		...
		foreach($this->A_Teams[$v_TeamName][$v_MemberID]['abilities'] as $v_ConditionType=>$v_ConditionName){
			switch($v_ConditionName){
				...
				/* Area Of Effect */
				case 'Aura Of Decay':
					$this->f_NewApplyComponents($v_TeamName,$v_MemberID,'magic',array('Challenge','Area','Close Range','Hostile','Practiced','Decay Effect','Slower Recovery','Increased Cost','Powered-By-Will','Anti-Health'));
					break;
				case 'Aura Of Death':
					$this->f_NewApplyComponents($v_TeamName,$v_MemberID,'magic',array('Challenge','Area','Close Range','Hostile','Practiced','Death Effect','Slowest Recovery','Very Increased Cost','Powered-By-Will','Anti-Health'));
					break;
				case 'Dazzling Aura':
				...
				case 'Rejuvenate':
					...
					break;
				case 'Giant Growth':
					$this->f_NewApplyComponents($v_TeamName,$v_MemberID,'magic',array('Enchantment','Close Range','Friendly','Instant','Practiced','Giant Growth Buff','Increased Cost','Slower Recovery'));
					break;
				...
				case 'Ghostly Wail':
					...
					break;
				case 'Wraith Cry':
					$this->f_NewApplyComponents($v_TeamName,$v_MemberID,'action',array('Challenge','Area','Close Range','Hostile','Practiced','Fear Effect','Adept Level','Powered-By-Will'));
					break;
				case 'Ground Pound':
				...
				case 'Thunderous Blast':
					$this->f_NewApplyComponents($v_TeamName,$v_MemberID,'magic',array('Challenge','Close Range','Hostile','Area','Practiced','Thunder-Blast Effect','Elongated Duration','Lesser Level','Powered-By-Will','Must-Stand'));
					break;
				...
				case 'Sonic Snap':
					...
					break;
				case 'Vampiric Bite':
					$this->f_NewApplyComponents($v_TeamName,$v_MemberID,'action',array('Challenge','Vampiric Bite Image','Vampiric Effect','Close Range','Hostile','Instant','Practiced','Target'));
					break;
				...
				case 'Throw Poison Flask':
					$this->f_NewApplyComponents($v_TeamName,$v_MemberID,'action',array('Poison Flask Image','Deflection','Close Range','Hostile','Area','Practiced','Poison Damage','Lesser Level','Amateur Power','Finesse','Must Dodge'));
					break;
				case 'Spit Poison':
				...
				case 'Slicing Strike':
					...
					break;
				case 'Strong Shot':
					$this->f_NewApplyComponents($v_TeamName,$v_MemberID,'action',array('Deflection','Near-By Range','Deadly','Powerful Left-Hand Damage','Practiced','Target','Projectile','Master Power','Overpowering'));
					break;
				case 'Strong Strike':
	...
	function f_AddEffect($v_TeamName,$v_MemberID,$v_PerformanceName,$v_Type,$v_EffectName){
		...
		switch($v_EffectName){
			...
			case 'Adrenaline Rush':case 'Bandage':case 'Decay':case 'Focused':case 'Heal':case 'Increased Size':case 'Rejuvenate':case 'Retribution':case 'Stoneskin':
				...
				break;
			case 'Afraid':case 'Blood Drain':case 'Chilled To The Bone':case 'Deafened':case 'Knocked Out':case 'Light Bruising':case 'Moderate Poisoning':case 'Shallow Gash':case 'Shallow Puncture':case 'Slight Burn':case 'Slight Daze':case 'Slight Shock':case 'Stunned':
			...
			case 'Blind':case 'Dazzle':case 'Knocked Down':case 'Lured':case 'Seriously Frozen':case 'Tripped':case 'Weak Poisoning':
				...
				break;
			case 'Deaf And Down':case 'Death':case 'Knocked Off-Balance':case 'Slightly Frozen':case 'Turn Undead':
	...
	function f_ApplyEffect($v_SourceTeamName,$v_SourceMemberID,$v_TeamName,$v_MemberID,$v_EffectName,$v_Duration){
		$a_Member=$this->A_Teams[$v_TeamName][$v_MemberID];
		$v_MemberLogName=$a_Member['name'];
		$v_Pass=true;
		switch($v_EffectName){
			case 'Tripped':case 'Knocked Out':case 'Knocked Down':case 'Knocked-Off Balance':
				...
				break;
			case 'Dazzle':case 'Decay':case 'Death':if(substr_count($a_Member['keywords'],'Alive')==0){$v_Pass=false;}break;
			...
			case 'Incineration':
				...
				if($v_ModifiedDamageTotal=$this->A_Teams[$v_TeamName][$v_MemberID]['health']['current']<=0){return true;}
				return false;
				break;
		...
		/* If the pass has failed, then the effect is resisted, else the effect is applied. */
		...
		switch($v_EffectName){
			...
			case 'Turn Undead':return $this->f_ChallengeTarget($v_SourceTeamName,$v_SourceMemberID,$v_TeamName,$v_MemberID,'action');break;
			case 'Death':return $this->f_ChallengeTarget($v_SourceTeamName,$v_SourceMemberID,$v_TeamName,$v_MemberID,'magic');break;
			...
			case 'Deaf And Down':
				...
				$this->A_Teams[$v_TeamName][$v_MemberID]['skills']['Block']-=4;
				$this->A_Teams[$v_TeamName][$v_MemberID]['skills']['Dodge']-=4;
			case 'Deafened':
				$this->A_Teams[$v_TeamName][$v_MemberID]['can-detect']=false;
				break;

			/* Knock-Down */
			case 'Knocked Out':case 'Dazzle':case 'Stunned':
		...
		*/
		...
		if($v_CriticalName!==''){
			...
			if($v_ApplyCritical){
				...
				if(!$v_CounterEffect){
					$v_Killed=$this->f_ApplyEffect($v_TeamName,$v_MemberID,$v_DefendingTeamName,$v_DefendingMemberID,$v_CriticalName,$v_CriticalDuration);
		...
		if($v_EffectName!==''&&!$v_Pass){
			...
			if($v_ApplyEffect){
				...
				/* Apply Or Counter Effect */
				if(!$v_CounterEffect){
					$v_Killed=$this->f_ApplyEffect($v_TeamName,$v_MemberID,$v_DefendingTeamName,$v_DefendingMemberID,$v_EffectName,$v_EffectDuration);
					if($v_Killed){return true;}
				}
			}
							...
							switch($v_PoolName){
								case 'blood':
									if(!array_key_exists('Unconscious',$this->A_Teams[$v_TeamName][$v_MemberID]['conditions'])){
										$this->f_ApplyEffect($v_TeamName,$v_MemberID,$v_TeamName,$v_MemberID,'Unconscious',60);
								...
								case 'energy':
									if(!array_key_exists('Exhausted',$this->A_Teams[$v_TeamName][$v_MemberID]['conditions'])){
										$this->f_ApplyEffect($v_TeamName,$v_MemberID,$v_TeamName,$v_MemberID,'Exhausted',3);
								...
								case 'focus':
									if(!array_key_exists('Confounded',$this->A_Teams[$v_TeamName][$v_MemberID]['conditions'])){
										$this->f_ApplyEffect($v_TeamName,$v_MemberID,$v_TeamName,$v_MemberID,'Confounded',6);
	...
	function f_LookAround($v_TeamName,$v_MemberID){
		...
		switch($v_RangeOfView){
			case 4:$a_OthersInView['distant']=$this->f_LookAtTile($v_TeamName,$v_MemberID,'distant',$v_Tile,3);
			case 3:$a_OthersInView['long']=$this->f_LookAtTile($v_TeamName,$v_MemberID,'long',$v_Tile,2);
			case 2:$a_OthersInView['short']=$this->f_LookAtTile($v_TeamName,$v_MemberID,'short',$v_Tile,1);
			case 1:$a_OthersInView['close']=$this->f_LookAtTile($v_TeamName,$v_MemberID,'close',$v_Tile,0);
			case 0:default:
				break;
		}
							...
							switch($v_RangeName){
								case 'short':
									if($a_Member['retreating']!==$a_DefendingMember['retreating']&&$a_Member['facing-right']!==$a_DefendingMember['facing-right']){
										$this->f_ApplyEffect($v_TeamName,$v_MemberID,$v_TeamName,$v_MemberID,'Defensive Posture',2);
		...
		foreach($a_Member['performance-preference'] as $v_PerformanceName=>$v_PreferenceWeight){
			if(!$a_Member[$v_PerformanceName]['has-target']&&$a_Member['can-'.$v_PerformanceName]){
				...
				if($v_Pass){
					...
					if($v_Pass){$a_Member['preferred-targets'][$v_TotalWeight][]=array($v_PerformanceName=>-1);}
				}
			}
		}
						...
						if($a_Member[$v_PerformanceName]['buff']){
								...
								if($a_Member[$v_PerformanceName]['hostile']){
									...
									foreach($this->A_Teams[$v_DefendingTeamName] as $v_TargetMemberID=>$a_TargetMember){
										if($a_TargetMember['location']==$a_Member['location']){
											...
											$v_Killed=$this->f_ApplyEffect($v_TeamName,$v_MemberID,$v_DefendingTeamName,$v_TargetMemberID,$v_EffectName,$v_EffectDuration);
											if($v_Killed){
								...
								/* Friendly */
									...
									foreach($this->A_Teams[$v_TeamName] as $v_TargetMemberID=>$a_TargetMember){
										if($a_TargetMember['location']==$a_Member['location']){
											...
											$v_EffectDuration=$a_Member[$v_PerformanceName]['effect']['duration'];
											$v_EffectName=$a_Member[$v_PerformanceName]['attacker']['effect-name'];
											$v_Killed=$this->f_ApplyEffect($v_TeamName,$v_MemberID,$v_TeamName,$v_TargetMemberID,$v_EffectName,$v_EffectDuration);
											if($v_Killed){
												$v_DamageType=$this->A_Teams[$v_TeamName][$v_MemberID][$v_PerformanceName]['damage']['type'];
												$this->f_AddStatistic($v_TeamName,$v_MemberID,'kills-by-character',$v_DamageType); /* Statistics */
							...
							/* Free-Form */
								...
								/* Friendly */
									...
									$v_EffectDuration=$a_Member[$v_PerformanceName]['effect']['duration'];
									$v_EffectName=$a_Member[$v_PerformanceName]['attacker']['effect-name'];
									$v_Killed=$this->f_ApplyEffect($v_TeamName,$v_MemberID,$v_TeamName,$v_MemberID,$v_EffectName,$v_EffectDuration);
									if($v_Killed){
										$v_DamageType=$this->A_Teams[$v_TeamName][$v_MemberID][$v_PerformanceName]['damage']['type'];
										$this->f_AddStatistic($v_TeamName,$v_MemberID,'kills-by-character',$v_DamageType); /* Statistics */
	...
	function f_PromoteToRank($v_TeamName,$v_MemberID,$v_Rank){
		...
		/* Soldier - Centurion - Legionary - Veteran - Commander */
		switch($v_Rank){
			case 'Commander':case 'Veteran':case 'Legionary':case 'Centurion':case 'Soldier':
	...
	function f_AddMemberToTeam($v_TeamName,$a_Parameters){
		...
		$this->f_PromoteToRank($v_TeamName,$v_MemberID,$v_Rank);
		$this->f_ApplyConditionsToMember($v_TeamName,$v_MemberID);
		$this->f_CalculateRatePerHour($v_TeamName,$v_MemberID);
		$this->f_SetCans($v_TeamName,$v_MemberID);
		$this->A_TeamData[$v_TeamName]['total-members-added']++;
		$this->f_CheckEmptyStatus($v_TeamName);
	}
	...
	function f_RemoveMemberFromTeam($v_TeamName,$v_MemberID){
		$a_Member=$this->A_Teams[$v_TeamName][$v_MemberID];
		$v_OnDeathEffect=$a_Member['on-death-effect'];
		...
		switch($v_OnDeathEffect){
			case 'Decaying Flesh': /* 22 - Skeleton Slasher */
				$this->f_AddMemberToTeam($v_TeamName,array(22,'Soldier',$v_EffectLocation));
				break;
			case 'Mummy Wrappings': /* 43 - Skeleton Penetrator */
				$this->f_AddMemberToTeam($v_TeamName,array(43,'Soldier',$v_EffectLocation));
				break;
			case '':default:
				break;
		}
	...
	function f_CheckGoal($v_TeamName){
		...
		switch($this->A_Party['goals'][$this->A_Party['current-goal']]){
			case 'no-opposing-members-left':
				if($this->A_Quest['opposing-members-left']<1){$v_Pass=true;}
				break;
			case 'one-reached-end':
				foreach($this->A_Teams[$v_TeamName] as $v_MemberID=>$a_Member){
					//$v_MemberLogName='[tile:'.$a_Member['location'].'] [id:'.$v_MemberID.'] '.$a_Member['name'];
					$v_MemberLogName=$a_Member['name'];
					if($a_Member['location']==$this->A_Quest['length-of-path']){$v_Pass=true;}
				}
				break;
			case 'one-reached-exit':
				foreach($this->A_Teams[$v_TeamName] as $v_MemberID=>$a_Member){
					//$v_MemberLogName='[tile:'.$a_Member['location'].'] [id:'.$v_MemberID.'] '.$a_Member['name'];
					$v_MemberLogName=$a_Member['name'];
					if($a_Member['location']==1){$v_Pass=true;}
				}
				break;
			default:
				break;
	...
	function f_ConstructColosseumTeamData($v_TeamName,$v_MemberLimit=1,$v_PointLimit=0){
		//if($v_PointLimit>0){$v_Points=true;}else{$v_PointsLeft=$v_PointLimit;}
		$this->A_TeamData[$v_TeamName]['data']=array();
		$a_NoPlayList=array(23,35,47,50,51,52,65);
		for($v_Counter=0;$v_Counter<$v_MemberLimit;$v_Counter++){
			for($v_FailSafe=0;$v_FailSafe<=10;$v_FailSafe++){
				$v_RandomMemberID=array_rand($this->A_TestMembers);
				if(!in_array($v_RandomMemberID,$a_NoPlayList)){break;}
			}
			//$v_PointsLeft-=$v_
			$this->A_TeamData[$v_TeamName]['data'][$v_Counter]=array($v_RandomMemberID,(($v_Counter>0)?'Veteran':'Commander'),1);
		}
	}
	function f_LoadTeam($v_TeamName){
		switch($v_TeamName){
			...
			case 'party':
				if($this->A_Quest['ID']==101){
					$this->f_ConstructColosseumTeamData($v_TeamName,1);
				}elseif($this->A_Quest['ID']==105){
					$this->f_ConstructColosseumTeamData($v_TeamName,6);
				}elseif($this->A_Quest['ID']==106){
					$this->f_ConstructColosseumTeamData($v_TeamName,12);
				}else{
					$this->A_TeamData[$v_TeamName]['data']=array(
						/* TEAM */
		...
		//foreach($this->A_TeamData[$v_TeamName]['data'] as $v_Key=>$a_Parameters){$this->f_AddCharacterToTeam($v_TeamName,$a_Parameters);}
	}
	...
	private $A_TestQuestData=array(
		...
		101=>array(
			...
			'ID'=>101,
			'name'=>'Colosseum',
			'description'=>'Test battle pitting random mercenary vs. random mercenary. NOTE: No Ancient Red Dragon, Goliath Warrior, Haunting Ghost, Lich King, Twisted Shapeshifter, Wisp Dancer or Shadow Wraith.',
			'image'=>0,
			'triggered-events'=>array(),
			'length-of-path'=>5,
			'maximum-turns-fail-safe'=>120,
		...
		105=>array(
			...
			'ID'=>105,
			'name'=>'Colosseum II',
			'description'=>'Test battle pitting six random mercenaries vs. six random mercenaries. NOTE: No Ancient Red Dragon, Goliath Warrior, Haunting Ghost, Lich King, Twisted Shapeshifter, Wisp Dancer or Shadow Wraith.',
			'image'=>0,
			'triggered-events'=>array(),
			'length-of-path'=>5,
			'maximum-turns-fail-safe'=>120,
		...
		106=>array(
			...
			'ID'=>106,
			'name'=>'Colosseum III',
			'description'=>'Test battle pitting twelve random mercenaries vs. twelve random mercenaries. NOTE: No Ancient Red Dragon, Goliath Warrior, Haunting Ghost, Lich King, Twisted Shapeshifter, Wisp Dancer or Shadow Wraith.',
			'image'=>0,
			'triggered-events'=>array(),
			'length-of-path'=>5,
			'maximum-turns-fail-safe'=>240,
		...
		16=>array(
			...
			'opposing-members-left'=>0,

			'starting-player-data'=>array(),
			'starting-data'=>array(),
			'ID'=>16,
			'name'=>'Test X',
			'description'=>'Calculating wages.',
			'image'=>1,
			'triggered-events'=>array(),
			'length-of-path'=>12,
			'maximum-turns-fail-safe'=>11,
			'minutes-per-turn'=>5,
			'seconds-per-turn'=>0,
			'preset-finish'=>0
		)
	...
	private $A_TestMembers=array(
		...
		7=>array(
			...
			'training'=>array('basic'=>'Zombie Block'),
		...
		23=>array(
			...
			'name'=>'Lich King',
			'guild'=>'Undead',
			'abilities'=>array('magic'=>'Aura Of Decay'),
			'traits'=>array('gender'=>'Male','race'=>'Lich','guild'=>'Undead'),
			'training'=>array('basic'=>'Novice Wisdom','minor'=>'Adept Wisdom','major'=>'Master Wisdom','specialization'=>'Lich Ruler'),
			'items'=>array('left-hand'=>'Staff Of Voices','feet'=>'Soul-Trapped Sandals','hands'=>'Soul-Trapped Handwraps','head'=>'Decaying Hat','legs'=>'Decaying Pantaloons','torso'=>'Decaying Robe'),
			'performance-preference'=>array('action'=>15,'attack'=>10,'magic'=>20,'move'=>0,'use'=>10),
		...
		*/
		...
		26=>array(
			...
			'items'=>array('left-hand'=>'Oak Staff','feet'=>'Heavy Cloth Sandals','hands'=>'Heavy Cloth Handwraps','head'=>'Heavy Cloth Hat','legs'=>'Heavy Cloth Pantaloons','torso'=>'Heavy Cloth Shirt'),
			'performance-preference'=>array('action'=>20,'attack'=>15,'magic'=>5,'move'=>0,'use'=>10),
			'language'=>array('Common','Dwarvish')
		),
		27=>array(
		...
		33=>array(
			...
			'training'=>array('basic'=>'Elven Intellect','minor'=>'Novice Archer','major'=>'Adept Archer','specialization'=>'Master Archer'),
			'items'=>array('left-hand'=>'Oak Light Crossbow','feet'=>'Leather Shoes','hands'=>'Leather Gloves','head'=>'Leather Cloak','legs'=>'Heavy Cloth Pantaloons','torso'=>'Heavy Cloth Shirt'),
			'stance'=>'',
			'performance-preference'=>array('action'=>20,'attack'=>15,'magic'=>5,'move'=>0,'use'=>10),
			'language'=>array('Common','Elvish')
		),
		...
		37=>array(
			...
			'training'=>array('basic'=>'Novice Tracker','minor'=>'Novice Archer','major'=>'Adept Archer','specialization'=>'Master Archer'),
		...
		43=>array(
			...
			'traits'=>array('gender'=>'Male','race'=>'Skeleton','guild'=>'Undead'),
			'training'=>array('basic'=>'Novice Wisdom','minor'=>'Adept Wisdom','major'=>'Novice Caster','specialization'=>'Adept Caster'),
			'stance'=>'stay-back',
			'items'=>array('left-hand'=>'Cedar Half-Staff','feet'=>'','hands'=>'','head'=>'Cloth Hat','legs'=>'','right-hand'=>'Cedar Half-Staff','torso'=>''),
			'performance-preference'=>array('action'=>15,'attack'=>10,'magic'=>20,'move'=>0,'use'=>5),
			'language'=>array('Undead')
		),
		44=>array(
		...
		45=>array(
			...
			'training'=>array('basic'=>'Novice Archer','minor'=>'Adept Archer','major'=>'Master Archer'),
			'stance'=>'stay-back',
		...
		53=>array(
			...
			'stance'=>'protect-group',
			'items'=>array('left-hand'=>'Spiked Knuckles','feet'=>'Iron Waders','hands'=>'Iron Glovelettes','head'=>'Iron Mantle','legs'=>'Leather Pants','torso'=>'Iron Ringmail'),
			'performance-preference'=>array('action'=>20,'attack'=>15,'magic'=>5,'move'=>0,'use'=>10),
			'language'=>array('Ogre')
		),
		54=>array(
		...
		60=>array(
			...
			'name'=>'Elven Archmage',
			'guild'=>'Wizard',
			'abilities'=>array('magic'=>'Thunderous Blast'),
		...
		61=>array(
			...
			'stance'=>'stay-back',
			'traits'=>array('gender'=>'Male','race'=>'Lich','guild'=>'Undead'),
			'training'=>array('basic'=>'Novice Gladiator','minor'=>'Adept Gladiator','major'=>'Master Gladiator','specialization'=>'Novice Block'),
			'items'=>array('left-hand'=>'Oak Staff','feet'=>'Black Cloth Sandals','hands'=>'Black Cloth Handwraps','head'=>'Black Cloth Hat','legs'=>'Black Cloth Pantaloons','torso'=>'Black Cloth Shirt'),
			'performance-preference'=>array('action'=>15,'attack'=>10,'magic'=>20,'move'=>0,'use'=>10),
			'language'=>array('Common','Undead')
		),
		62=>array(
			...
			'guild'=>'Undead',
			'abilities'=>array('action'=>'Unexpected Strike'),
			'traits'=>array('gender'=>'Female','race'=>'Skeleton','guild'=>'Rogue'),
			'training'=>array('basic'=>'Skeleton Balance','minor'=>'Novice Marksman','major'=>'Adept Marksman'),
			'stance'=>'',
			'items'=>array('left-hand'=>'Cedar Nunchuck','feet'=>'Black Cloth Sandals','hands'=>'Black Cloth Handwraps','head'=>'Black Cloth Hat','legs'=>'Leather Pants','torso'=>'Leather Tunic'),
			'performance-preference'=>array('action'=>20,'attack'=>15,'magic'=>5,'move'=>0,'use'=>10),
			'language'=>array('Undead')
		),
		63=>array(
			...
			'abilities'=>array('action'=>'Moan And Groan'),
			'traits'=>array('gender'=>'Male','race'=>'Zombie','guild'=>'Undead'),
			'training'=>array('basic'=>'Decaying Flesh'),
			'stance'=>'protect-group',
			'items'=>array('left-hand'=>'Iron Longsword','feet'=>'Leather Shoes','hands'=>'Leather Gloves','head'=>'Leather Cloak','legs'=>'Leather Pants','torso'=>'Leather Tunic'),
			'performance-preference'=>array('action'=>20,'attack'=>15,'magic'=>5,'move'=>0,'use'=>10),
			'language'=>array('Undead')
		),
		64=>array(
			...
			'name'=>'Mummy Servant',
			'guild'=>'Undead',
			'abilities'=>array('action'=>'Moan And Groan','magic'=>'Magic Missile'),
			'traits'=>array('gender'=>'Male','race'=>'Mummy','guild'=>'Undead'),
			'training'=>array('basic'=>'Mummy Wrappings'),
			'stance'=>'protect-group',
			'items'=>array('left-hand'=>'Wand Of Magic Missile','feet'=>'Leather Shoes','hands'=>'Leather Gloves','head'=>'Leather Cloak','legs'=>'Leather Pants','torso'=>'Leather Tunic'),
			'performance-preference'=>array('action'=>20,'attack'=>10,'magic'=>15,'move'=>0,'use'=>5)
		),
		65=>array(
			'id'=>65,
			'name'=>'Shadow Wraith',
			'guild'=>'Undead',
			'abilities'=>array('action'=>'Wraith Cry'),
			'traits'=>array('gender'=>'Male','race'=>'Wraith','guild'=>'Undead'),
			'training'=>array('basic'=>'Novice Sneak','minor'=>'Adept Sneak','major'=>'Novice Wisdom','specialization'=>'Adept Wisdom'),
			'stance'=>'protect-group',
			'items'=>array('left-hand'=>'Cold Ghostly Strands'),
			'performance-preference'=>array('action'=>20,'attack'=>10,'magic'=>15,'move'=>0,'use'=>5)
		)
