...
var A_Constructs={
	'materials':{
		...
		'header':{
			...
			'ready':{'class':'font-bold font-X','text':''}
		},
		'list-ID':0,
		'line-replace':{
			'one':'name',
...
function F_CloneLineAndInsertData(v_AreaName,a_Data){
	var o_Line=$('#line-template').clone().attr('id',v_AreaName+'-line-'+a_Data['counter']);
	o_Line.find('[id|="replace"]').each(function(v_Event){
		var v_ReplaceKey=$(this).attr('id').replace('replace-','');
		switch($(this).attr('name')){
			case 'normal':default:
				$(this).html(a_Data[A_Constructs[v_AreaName]['line-replace'][v_ReplaceKey]]);
				break;
		}
	});
	o_Line.show();
...
function F_CloneOpenLineAndInsertData(v_AreaName,a_Data){
	...
	o_OpenLine.find('[id|="replace"]').each(function(v_Event){
		...
		switch($(this).attr('name')){
			case 'normal':default:
				$(this).html(a_Data[A_Constructs[v_AreaName]['open-line-replace'][v_ReplaceKey]]);
				break;
		}
	});
	return o_OpenLine;
...
function F_CloneConstruct(v_AreaName){
	var o_Construct=$('#construct-template').clone().attr('id',v_AreaName+'-construct');
	o_Construct.bind('append-list',function(v_Event){
		var a_Data=A_Constructs[v_AreaName]['data'];
		o_Construct.find('#list-data').html('');
		for(var v_Key in a_Data){o_Construct.find('#list-data').append(F_CloneLineAndInsertData(v_AreaName,a_Data[v_Key]));}
	});
	o_Construct.bind('change-header',function(v_Event,v_Mode){$(this).find('#header-text').attr('class',A_Constructs[v_AreaName]['header'][v_Mode]['class']).html(A_Constructs[v_AreaName]['header'][v_Mode]['text']);});
	o_Construct.bind('close-line',function(v_Event){o_Construct.find('#open-line-left').css('width','100%');o_Construct.find('#open-line-right').css('width',0);o_Construct.find('#'+v_AreaName+'-open-line').hide();});
	o_Construct.bind('expand-or-shrink',function(v_Event,v_Mode){
		A_Constructs[v_AreaName]['size']=v_Mode;
		$(this).css({'height':A_Constructs[v_AreaName][v_Mode]['height'],'width':A_Constructs[v_AreaName][v_Mode]['width']});
		o_Construct.trigger('update-control-icons');
		o_Construct.trigger(((v_Mode=='expand')?'open':'close')+'-line');
		F_HighlightMenu(v_Mode,v_AreaName);
	});
	o_Construct.bind('highlight-target',function(v_Event){
		var a_Data=A_Constructs[v_AreaName]['data'];
		var v_Target=A_Constructs[v_AreaName]['target'];
		o_Construct.find('.open-line').removeClass('open-line').addClass('button-Y-35');
		o_Construct.find('#'+v_AreaName+'-line-'+v_Target).removeClass('button-Y-35').addClass('open-line');
		o_Construct.find('#open-line-right').html(F_CloneOpenLineAndInsertData(v_AreaName,a_Data[v_Target]));
	});
	o_Construct.bind('open-line',function(v_Event){o_Construct.find('#open-line-left').css('width',580);o_Construct.find('#open-line-right').css('width',380);o_Construct.find('#'+v_AreaName+'-open-line').show();});
	o_Construct.bind('request-blind-URL',function(v_Event){
	...
	o_Construct.bind('show-or-hide',function(v_Event,v_Mode){switch(v_Mode){case 'hide':$(this).hide();break;default:case 'show':$(this).show();break;}});
	o_Construct.bind('update-control-icons',function(v_Event){
		o_Construct.find('#control-icons img').each(function(){
	...
	o_Construct.bind('select-from-list',function(v_Event){
		$('#black-screen').show();
	...
	o_Construct.find('#control-expand').live('click',function(v_Event){o_Construct.trigger('expand-or-shrink','expand');});
	o_Construct.find('#control-shrink').live('click',function(v_Event){o_Construct.trigger('expand-or-shrink','shrink');});
	o_Construct.find('#button-lists').live('click',function(v_Event){o_Construct.trigger('select-from-list');});
	o_Construct.find('[id|="'+v_AreaName+'-line"]').live('click',function(v_Event){
		A_Constructs[v_AreaName]['target']=$(this).attr('id').replace(v_AreaName+'-line-','');
		o_Construct.trigger('highlight-target').trigger('expand-or-shrink','expand');
	});

	o_Construct
		.appendTo('#destination-content')
		.trigger('change-header',A_Constructs[v_AreaName]['status'])
		.trigger('expand-or-shrink',A_Constructs[v_AreaName]['size'])
...
function F_StartTest(v_X,v_Y){
	F_OpenBlackScreen();
	F_OpenListChanger(v_X,v_Y,A_MaterialLists,4);
}
		...
		switch(a_Elements[v_Key]){
			...
			case 'list-changer':case 'list-options':
				...
				if(v_ShowArrows){
					...
					$('<img/>',{'class':'button-Y-35 shadow-or-light-X-down','css':{'height':16,'position':'absolute','right':4,'top':4,'width':16,'z-index':70},'src':'http://cdn1.lapcat.org/fugue/icons/cross-small.png'}).click(function(){F_CloseBlackScreen();return false;}).appendTo('#'+a_Elements[v_Key]+'-inside');
				}
				break;
			default:
				break;
...
function F_OpenBlackScreen(){A_ElementsOnScreen['black-screen']=true;$('#black-screen').show();}
function F_OpenListChanger(v_X,v_Y,a_Data,v_MaximumLines){
	A_ElementsOnScreen['list-changer']=true;
	$('#list-changer-lines').html(F_GetListChangerHTML('list-changer',a_Data,v_MaximumLines));
	$('#list-changer').css('left',v_X).css('top',v_Y).show();
	A_ElementsOnScreen['list-options']=true;
	$('#list-options-lines').html(F_GetListOptionsHTML('list-options',a_Data,v_MaximumLines));
	$('#list-options').css('left',v_X+333).css('top',v_Y).show();
	F_ChangeListLine();
}
function F_RequestMaterialList(){
	$('#display-materials #base-list-master').fadeTo(0,0.25);
	$('#display-header-text-materials').html($('<font/>',{'class':'font-italic font-I','html':'Loading...'}));
	V_MaterialListKey=V_CurrentListKey;
	$.getJSON('/quick/get-material-list/'+V_MaterialListKey,function(o_JSON){
		switch(o_JSON['switch']){
			case 'data':
				$('#display-header-text-materials').html(o_JSON['header']);
				A_DisplayData['materials']['data']=o_JSON['data'];
				A_DisplayData['materials']['target']=0;
				F_ManageData('materials','base-list','display-materials','list');
				if(A_DisplayData['materials']['size']=='expanded'){$('#display-materials').trigger('zoom-in');}
				break;
			case '':default:
				break;
		}
...
var A_DisplayData={
	'databases':{
		...
		'view':'list',
		'limits':{
			'list':{'shrunk':6,'expanded':10},
			'featured':{'shrunk':1,'expanded':1}
		},
		'icons':{
			'magnifier':0,
			'list':1
		...
		'icons-data':{
			'magnifier':{0:'magnifier-zoom-in',1:'magnifier-zoom-out'},
			'list':{0:'application-view-list',1:'layout-content'}
		},
		'search':{
			'date':0,
			'location':0
	...
	'events':{
		...
		'limits':{
			'list':{'shrunk':6,'expanded':10},
			'calendar':{'shrunk':100,'expanded':100}
		},
		'icons':{
			'magnifier':0,
			'list':1
		...
		'icons-data':{
			'magnifier':{0:'magnifier-zoom-in',1:'magnifier-zoom-out'},
			'list':{0:'application-view-list',1:'calendar'}
		},
		'search':{
			'date':0,
			'location':0
	...
	'materials':{
		...
		'open-line':{
			...
			'five':'ISBNorSN',
			'six':'summary',
			'seven':'available-home',
			'eight':'available-other',
			'nine':'ISBNorSN',
			'ten':'ISBNorSN'
		},
		'target':0,
		...
		'view':'list',
		'limits':{
			'list':{'shrunk':4,'expanded':10},
			'images':{'shrunk':6,'expanded':12}
		},
		'icons':{
			'magnifier':0,
			'list':1
		...
		'icons-data':{
			'magnifier':{0:'magnifier-zoom-in',1:'magnifier-zoom-out'},
			'list':{0:'application-view-list',1:'images'}
		},
		'search':{
			'date':0,
			'credit-name':0
...
/* Function - Construct Stage */
function F_ConstructStage(){

	/* Add Display Containers (to body) */
	$('<div/>',{
		'id':'containers',
		'css':{'height':V_OpenLineMaximumHeight,'width':V_MaximumWidth}
	}).appendTo('#destination-content');

	/* Add Display Container Top (to Display Containers) */
	$('<div/>',{
		'id':'container-top',
		'css':{'height':V_ShrunkTopRowMaximumHeight,'overflow':'hidden','padding-left':1,'padding-right':1,'vertical-align':'top','width':'100%'}
	}).appendTo('#containers');

	/* Add Display Container 1 (to Display Containers) */
	$('<div/>',{
		'id':'display-container-1',
		'css':{'height':V_ShrunkTopRowMaximumHeight,'overflow':'hidden','vertical-align':'top','width':'100%'}
	}).appendTo('#container-top');

	/* Add Display Container Bottom (to Display Containers) */
	$('<div/>',{
		'id':'container-bottom',
		'css':{'height':V_ShrunkBottomRowMaximumHeight,'padding-left':1,'padding-right':1,'vertical-align':'top','width':'100%'}
	}).appendTo('#containers');

	/* Add Display Container 2 (to Display Containers) */
	$('<div/>',{
		'id':'display-container-2',
		'css':{'display':'table-cell','height':V_ShrunkBottomRowMaximumHeight,'overflow':'hidden','vertical-align':'top','width':V_QuarterMaximumWidth}
	}).appendTo('#container-bottom');

	$('<div/>',{
		'css':{'display':'table-cell','height':V_ShrunkBottomRowMaximumHeight,'width':2}
	}).appendTo('#container-bottom');

	/* Add Display Container 3 (to Display Containers) */
	$('<div/>',{
		'id':'display-container-3',
		'css':{'display':'table-cell','height':V_ShrunkBottomRowMaximumHeight,'overflow':'hidden','vertical-align':'top','width':V_QuarterMaximumWidth}
	}).appendTo('#container-bottom');
	...
	/* Add Display Container 4 (to Display Containers) */
	$('<div/>',{
		'id':'display-container-4',
		'css':{'display':'table-cell','height':V_ShrunkBottomRowMaximumHeight,'overflow':'hidden','vertical-align':'top','width':V_QuarterMaximumWidth}
	}).appendTo('#container-bottom');
	...
	/* Add Display Container 5 (to Display Containers) */
	$('<div/>',{
		'id':'display-container-5',
		'css':{'display':'table-cell','height':V_ShrunkBottomRowMaximumHeight,'overflow':'hidden','vertical-align':'top','width':V_QuarterMaximumWidth}
	}).appendTo('#container-bottom');
	...
	/* Add Icons (to Stage) */
	var a_Icons=['application_view_list','building','building_delete','calendar','cross','date_delete','images','layout_content','magnifier_zoom_in','magnifier_zoom_out','user'];
	for(var v_Key in a_Icons){
		$('<div/>',{'id':'icon-'+(a_Icons[v_Key].replace(/_/g,'-')),'css':{'background-image':'url(http://cdn1.lapcat.org/famfamfam/silk/'+a_Icons[v_Key]+'.png)','float':'left','height':16,'width':16}}).appendTo('#stage');
	}

	/* Add Small Icon (to Stage) */
	$('<img/>',{'id':'small-icon-cross','src':'http://cdn1.lapcat.org/famfamfam/silk/cross.png','css':{'float':'left','height':12,'padding-left':2,'padding-top':2,'width':12}}).appendTo('#stage');

	/* Add Title Search (to Stage) */
	$('<div/>',{
	  	'id':'title-search'
	}).appendTo('#stage');
	
	/* Add Change Title Search (to Title Search) */
	$('<form/>',{
		'id':'change-title-search',
		'class':'color-Z-4 corners-top-2 name-search',
		'css':{'float':'left','height':14,'width':158},
		'autocomplete':'off',
		'method':'get',
		'title':'Click then type to change the title for this search.'
	}).appendTo('#title-search');

	/* Add Change Title Search Master (to Change Title Search) */
	$('<input/>',{
		'id':'change-title-search-master',
		'class':'drops corners-top-2 dropdown-A-1 similar-box fake-link',
		'type':'text',
		'value':'title search',
		'css':{'border-bottom':0,'float':'right','font-size':11,'height':12,'padding-left':3,'width':158}
	}).appendTo('#change-title-search');

	/* Add Similar Selected (to Title Search) */
	$('<div/>',{
		'id':'similar-selected',
		'css':{'display':'none','float':'left','width':16},
		'html':$('#small-icon-cross').clone().addClass('fake-link').attr('title','Click to remove this criteria form the search.')
	}).appendTo('#change-title-search');

	/* Add Base List Master (to Stage) */
	$('<div/>',{
		'id':'base-list-master','class':'font-fake font-X',
		'css':{'display':'table','height':'auto','width':'100%'}
	}).appendTo('#stage');

	/* Add Base List Container (to Base List Master) */
	$('<div/>',{
		'id':'base-list-container','class':'font-fake font-X',
		'css':{'display':'table-cell','height':'auto','width':'auto'}
	}).appendTo('#base-list-master');

	/* Add Base List Open Line (to Base List Master) */
	$('<div/>',{
		'id':'base-list-open-line','class':'font-fake font-X',
		'css':{'display':'table-cell','height':'auto','vertical-align':'top','width':0}
	}).appendTo('#base-list-master');

	/* Add Open Line Master (to Base List Open Line) */
	$('<div/>',{
		'id':'open-line-master','class':'font-fake font-X',
		'css':{'display':'none','height':'auto','vertical-align':'top','width':'100%'}
	}).appendTo('#base-list-open-line');

	/* Add Open Line Name (to Open Line Master) */
	$('<a/>',{
		'id':'replace-nine','name':'normal-catalog-link','href':'','target':'_blank',
		'html':$('<div/>',{
			'id':'replace-one','class':'button-blue-2 font-G font-bold inside-border shadow-or-light-Y-down','name':'normal',
			'css':{'background-position':'0px 22px','float':'left','font-size':15,'height':40,'margin-bottom':3,'margin-top':3,'padding-left':3,'max-height':40,'width':'86%'}
		})
	}).appendTo('#open-line-master');

	$('<div/>',{'css':{'float':'left','height':40,'margin-bottom':3,'margin-top':3,'max-height':40,'width':'1%'}}).appendTo('#open-line-master');

	/* Add Open Line Name (to Open Line Master) */
	$('<a/>',{
		'id':'replace-ten','name':'normal-request-link','href':'','target':'_blank',
		'html':$('<div/>',{
			'class':'button-blue-2 font-G inside-border shadow-or-light-Y-down','name':'normal',
			'css':{'background-position':'0px 2px','float':'left','font-size':10,'height':20,'margin-bottom':3,'margin-top':3,'padding-left':7,'padding-top':2,'max-height':20,'width':'13%'},
			'html':'request'
		})
	}).appendTo('#open-line-master');

	/* Add Open Line Availability (to Open Line Master) */
	$('<div/>',{
		'id':'replace-seven','name':'available-at-home',
		'css':{'float':'left','height':16,'max-height':16,'width':'6%'},
		'html':$('<img/>',{'src':'http://cdn1.lapcat.org/fugue/icons/tick-white.png','css':{'padding-left':6,'height':16,'width':16}})
	}).appendTo('#open-line-master');

	/* Add Open Line Availability (to Open Line Master) */
	$('<div/>',{
		'id':'replace-eight','name':'available-at-other',
		'css':{'float':'right','height':16,'max-height':16,'width':'6%'},
		'html':$('<img/>',{'src':'http://cdn1.lapcat.org/fugue/icons/tick-small-white.png','css':{'padding-right':6,'height':16,'width':16}})
	}).appendTo('#open-line-master');

	/* Add Open Line Secondary (to Open Line Master) */
	$('<div/>',{
		'id':'open-line-secondary','class':'border-bottom-I-1-65 corners-bottom-3 corners-top-3 font-Y shadow-or-light-X-up',
		'css':{'display':'block','float':'left','height':18,'padding-left':3,'max-height':18,'width':'100%'}
	}).appendTo('#open-line-master');

	$('<div/>',{
		'class':'font-I','id':'replace-two','name':'normal',
		'css':{'display':'block','float':'left','font-size':10,'height':20,'overflow':'hidden','padding-left':6,'text-align':'right','vertical-align':'top','white-space':'nowrap','width':'auto'}
	}).appendTo('#open-line-secondary');

	$('<div/>',{
		'id':'replace-three','name':'with-icon','class':'font-X',
		'css':{'display':'block','float':'left','font-size':12,'height':20,'overflow':'hidden','padding-left':6,'vertical-align':'top','width':'auto'}
	}).appendTo('#open-line-secondary');

	$('<div/>',{
		'id':'replace-four','name':'normal','class':'font-X',
		'css':{'display':'block','float':'right','font-size':12,'height':20,'overflow':'hidden','text-align':'right','padding-right':6,'vertical-align':'top','white-space':'nowrap','width':70}
	}).appendTo('#open-line-secondary');

	/* Add Open Line Primary (to Open Line Master) */
	$('<div/>',{
		'id':'open-line-primary','class':'border-top-I-1-65 corners-top-3 font-Y',
		'css':{'display':'block','float':'left','height':368,'padding-left':3,'max-height':368,'width':'100%'}
	}).appendTo('#open-line-master');

	$('<div/>',{
		'id':'replace-six','class':'font-X','name':'normal-float',
		'css':{'display':'block','float':'left','overflow':'hidden','padding-left':6,'padding-top':2,'vertical-align':'top','width':'99%'},
		'html':$('<div/>',{'id':'replace-five','name':'image','css':{'float':'left','height':'auto','padding-right':6,'width':'auto'}})
	}).appendTo('#open-line-primary');

	/* Add Base Line (to Base List) */
	for(var v_Counter=0;v_Counter<10;v_Counter++){

		$('<div/>',{
			'id':'base-line-'+v_Counter,
		  	'class':'button-Y-35 inside-border',
			'name':'clicked-line-'+v_Counter,
			'css':{'height':40,'margin-bottom':3,'margin-top':3,'max-height':40,'width':'100%'},
			'title':'Click to expand this display.'
		}).appendTo('#base-list-container');

		$('<div/>',{
			'id':'base-line-top-'+v_Counter,
			'css':{'display':'table','height':20,'width':'100%'}
		}).appendTo('#base-line-'+v_Counter);

		$('<div/>',{
			'id':'base-line-bottom-'+v_Counter,
			'css':{'display':'table','height':20,'width':'100%'}
		}).appendTo('#base-line-'+v_Counter);
		...
		$('<div/>',{
			'id':'replace-one',
			'name':'normal',
			'css':{'display':'table-cell','font-size':16,'height':20,'overflow':'hidden','padding-left':6,'vertical-align':'top','width':'100%'}
		}).appendTo('#base-line-top-'+v_Counter);

		$('<div/>',{
			'class':'font-I',
			'id':'replace-two',
			'name':'normal',
			'css':{'display':'table-cell','font-size':10,'height':20,'overflow':'hidden','padding-left':6,'text-align':'right','vertical-align':'top','white-space':'nowrap','width':'auto'}
		}).appendTo('#base-line-bottom-'+v_Counter);

		$('<div/>',{
			'id':'replace-three',
			'name':'with-icon',
			'css':{'display':'table-cell','font-size':12,'height':20,'overflow':'hidden','padding-left':6,'vertical-align':'top','width':'100%'}
		}).appendTo('#base-line-bottom-'+v_Counter);

		$('<div/>',{
			'id':'replace-four',
			'name':'normal',
			'css':{'display':'table-cell','font-size':12,'height':20,'overflow':'hidden','text-align':'right','padding-right':6,'vertical-align':'top','white-space':'nowrap','width':70}
		}).appendTo('#base-line-bottom-'+v_Counter);

	}

}
function F_AddMoreElements(){

	$('<div/>',{
		'id':'list-changer',
		'class':'color-Y-4 corners-bottom-2 corners-top-2 shadow-or-light-X-up',
		'css':{'display':'none','height':148,'left':0,'padding':3,'position':'relative','top':0,'width':440,'z-index':50}
	}).appendTo('body');

	$('<div/>',{
	  'id':'list-changer-inside',
	  'class':'border-all-gold-1 color-X-1 corners-bottom-2 corners-top-2 font-fake font-X shadow-or-light-X-up',
	  'css':{'float':'left','height':144,'padding-top':2,'width':318}
	}).appendTo('#list-changer');

	$('<div/>',{
	  'id':'list-changer-outside',
	  'class':'border-all-gold-1 color-X-1 corners-bottom-2 corners-top-2 font-fake font-X shadow-or-light-X-up',
	  'css':{'float':'left','height':144,'padding-left':6,'padding-top':2,'width':112}
	}).appendTo('#list-changer');

	$('<div/>',{'id':'new-screen','class':'color-K-1','css':{'display':'none','height':'100%','left':0,'position':'fixed','top':0,'width':'100%','z-index':45}})
		.click(function(){$('.change-list-click').click();return false;})
		.appendTo('body');
}
...
function F_GetValueFromData(v_Area,v_Depth,v_Counter,a_Keys){
	var v_Value='';
	if(v_Depth>0){
		if(v_Depth>1){
			if(v_Depth>2){
				v_Value=A_DisplayData[v_Area]['data'][v_Counter][a_Keys[0]][a_Keys[1]][a_Keys[2]];
			}else{
				v_Value=A_DisplayData[v_Area]['data'][v_Counter][a_Keys[0]][a_Keys[1]];
			}
		}else{
			v_Value=A_DisplayData[v_Area]['data'][v_Counter][a_Keys[0]];
		}
	}else{
		v_Value=A_DisplayData[v_Area]['data'][v_Counter];
	}
	return v_Value;
}

/* Function - Apply Data */
function F_ApplyData(v_Area,v_Counter,v_Type,v_Destination){
	var a_Keys='';
	var v_Key='one';
	var v_Depth=0;
	var v_Value=0;
	...
	$('#'+v_Destination)
		.show()
		.find('[id|="replace"]').each(function(){

			v_Key=$(this).attr('id').replace('replace-','');
			v_Value='';

			if(A_DisplayData[v_Area][v_Type][v_Key]!==''){

				a_Keys=A_DisplayData[v_Area][v_Type][v_Key].split('|');
				v_Depth=a_Keys.length;
				v_Value=F_GetValueFromData(v_Area,v_Depth,v_Counter,a_Keys);
				...
				switch($(this).attr('name')){
					case 'with-icon':
						...
						if(v_Value!==''){
							$(this).html(v_Value);
							switch(v_Area){
								case 'events':$(this).prepend($('#icon-building').clone().css('margin-right',2));break;
								case 'materials':case 'news':$(this).prepend($('#icon-user').clone().css('margin-right',2));break;
								case '':default:
									break;
							}
						}
						break;
					case 'available-at-home':case 'available-at-other':
						if(v_Value){$(this).show();}else{$(this).hide();}
						break;
					case 'normal-request-link':$(this).attr('href','https://catalog.lapcat.org:443/patroninfo~S12?/0/redirect=/search%7ES12?/i'+v_Value+'/i'+v_Value+'/1%2C1%2C1%2CE/request&FF=i'+v_Value+'&1%2C1%2C');break;
					case 'normal-catalog-link':$(this).attr('href','http://catalog.lapcat.org/search/i'+v_Value);break;
					case 'normal':$(this).html(v_Value);break;
					case 'normal-float':
						$('#open-line-primary-summary').replaceWith('');
						if(v_Value==''){
							$(this).append($('<span/>',{'id':'open-line-primary-summary','html':'<font class="font-L">Summary</font><br/><font class="font-I font-italic" style="font-size:12px;">No summary available.</font>'}));
						}else{
							$(this).append($('<span/>',{'id':'open-line-primary-summary','html':'<font class="font-L">Summary</font><br/>'+v_Value}));
						}
						break;
					case 'image':
						$('#image-holder').html('');
						$('#open-line-primary #replace-five').html('');
						F_RequestImage('large-image',A_DisplayData[v_Area]['data'],'L',A_DisplayData[v_Area]['target'],'replace-five');
						break;
					case '':default:
						break;
				}
...
function F_ManageData(v_Area,v_Element,v_Destination,v_Type){

	$('#display-materials #base-list-master').fadeTo(0,1.00);
	if(!$('#'+v_Destination+' #'+v_Element+'-master').length){
		$('#'+v_Element+'-master').clone().hide().appendTo('#'+v_Destination);
	}

	switch(v_Type){
		case 'list':
			var v_Size=A_DisplayData[v_Area]['size'];
			var v_View=A_DisplayData[v_Area]['view'];
			var v_Limit=A_DisplayData[v_Area]['limits'][v_View][v_Size];
			var v_Hide=false;
			for(var v_Counter=0;v_Counter<10;v_Counter++){
				v_Hide=false;
				if(F_ArrayKeyExists(v_Counter,A_DisplayData[v_Area]['data'])){
					F_ApplyData(v_Area,v_Counter,v_Type,v_Destination+' #base-line-'+v_Counter);
					if(v_Counter>=v_Limit){v_Hide=true;}
				}else{v_Hide=true;}
				if(v_Hide){$('#'+v_Destination+' #base-line-'+v_Counter).hide();}
			}
			break;
		case '':default:
			break;
	...
	$('#'+v_Destination+' #'+v_Element+'-master').show();

}

/* Function - Construct Display */
function F_ConstructDisplay(v_Area){

	var o_Display=$('<div/>',{
		'id':'display-'+v_Area,
		'class':'corners-top-3 font-fake font-X shadow-or-light-Y-up',
		'css':{'width':'100%'}
	}).appendTo('#display-container-'+A_DisplayData[v_Area]['container']);
	
	/* Add Display Header Table (to Display) */
	$('<div/>',{
		'id':'display-header-table-'+v_Area,
		'class':'color-X-4 corners-bottom-3 corners-top-3 shadow-or-light-X-down',
		'css':{'background-position':'0px 2px','height':18,'width':'100%'}
	}).appendTo('#display-'+v_Area);

	/* Add Display Header (to Display Header Table) */
	$('<div/>',{
		'class':'corners-bottom-2 corners-top-2 shadow-or-light-Y-down',
		'id':'display-header-'+v_Area,
		'css':{'display':'block','height':16,'width':'100%'}
	}).appendTo('#display-header-table-'+v_Area);

	/* Add Display Header Switch  (to Display Header) (for Materials) */
	if(v_Area=='materials'){

		$('<div/>',{
			'class':'corners-top-3 shadow-or-light-X-up',
			'id':'display-header-switch-'+v_Area,
			'css':{'display':'block','float':'left','height':16,'padding-left':8,'padding-right':8,'padding-top':1,'width':26},
			'html':$('<div/>',{'id':'button-lists','class':'button-gold font-K inside-border shadow-or-light-Y-down change-list-click','css':{'display':'none','font-size':10,'position':'absolute','text-align':'center','width':26,'z-index':65},'html':'List'})
		}).appendTo('#display-header-'+v_Area);

		$('<div/>',{
			'id':'display-header-switch-'+v_Area,
			'css':{'display':'block','float':'left','height':16,'padding-left':2,'padding-right':2,'padding-top':2,'width':26},
			'html':$('<img/>',{'src':'http://cdn1.lapcat.org/fugue/icons/control.png','css':{'height':8,'margin-bottom':4,'width':8}})
		}).appendTo('#display-header-'+v_Area);

	}
	
	/* Add Display Header Text (to Display Header) */
	$('<div/>',{
		'class':'font-bold',
		'id':'display-header-text-'+v_Area,
		'css':{'display':'block','float':'left','font-size':13,'height':16,'overflow':'hidden','padding-left':3,'width':'auto'}
	}).appendTo('#display-header-'+v_Area);

	$('<div/>',{
		'class':'corners-bottom-3 corners-top-3 shadow-or-light-X-up',
		'id':'display-header-icons-'+v_Area,
		'css':{'display':'block','float':'right','height':16,'padding-right':3,'width':90}
	}).appendTo('#display-header-'+v_Area);

	/* Add Display Header Search (to Display Header) (for Events and Materials) */
	/*
	if(v_Area=='events'||v_Area=='materials'){

		$('<div/>',{
			'css':{'display':'block','float':'right','height':16,'width':3}
		}).appendTo('#display-header-'+v_Area);

		$('<div/>',{
			'class':'corners-top-3 shadow-or-light-X-up',
			'id':'display-header-search-'+v_Area,
			'css':{'display':'none','float':'right','height':16,'padding-left':12,'padding-top':2,'width':180}
		}).appendTo('#display-header-'+v_Area);

	}
	*/
	
	/* Show Search (for Materials) */
	if(v_Area=='materials'){$('#display-header-search-'+v_Area).css('display','block');}

	o_Display.bind('highlight-target',function(){
		$(this).find('.open-line').removeClass('open-line').addClass('button-Y-35');
		$(this).find('#base-line-'+A_DisplayData[v_Area]['target']).removeClass('button-Y-35').addClass('open-line');
	});
	
	o_Display.bind('show-navigation-icons',function(){

		switch(v_Area){
			case 'events':case 'materials':
				$('#title-search').clone().appendTo('#display-header-search-'+v_Area);
				break;
			case '':default:
				break;
		}

		for(var v_Key in A_DisplayData[v_Area]['icons']){
			switch(v_Key){
				case 'list':case 'magnifier':
					$('#icon-'+A_DisplayData[v_Area]['icons-data'][v_Key][A_DisplayData[v_Area]['icons'][v_Key]]).clone().addClass('fake-link').css('float','right').appendTo('#display-header-icons-'+v_Area);
					$('<div/>',{'class':'color-I-1','css':{'float':'right','height':13,'margin':2,'width':1}}).appendTo('#display-header-icons-'+v_Area);
					break;
				case '':default:
					break;
			}
	...
	o_Display.bind('open-target',function(){
		$(this).find('#base-list-open-line').css('width',400).css('padding',3);
		$(this).find('#open-line-master').css('display','table');
		F_ApplyData(v_Area,A_DisplayData[v_Area]['target'],'open-line','open-line-master');
	});
	
	o_Display.bind('zoom-in',function(){
		A_DisplayData[v_Area]['size']='expanded';
		var v_Container=A_DisplayData[v_Area]['container'];
		$('#container-top').css('height',V_OpenLineMaximumHeight);
		$('#display-container-'+v_Container).css('height',V_OpenLineMaximumHeight);
		$('#display-container-'+v_Container+' [id|="base-line"]').each(function(v_Index){
			if(F_ArrayKeyExists(v_Index,A_DisplayData[v_Area]['data'])){$('#display-container-'+v_Container+' #base-line-'+v_Index).show();}
		});
		if(v_Container>1){$('#container-top').hide();$('#container-bottom').show();}else{$('#container-bottom').hide();$('#container-top').show();}
		/* Hide Other Containers */
		$('[id|="display-container"]:not(#display-container-'+v_Container+')').hide();
		/* Highlight Target / Open Target */
		$(this)
			.trigger('highlight-target')
			.trigger('open-target');
	});
	
	o_Display.bind('request-blind-URL',function(){
		$.getJSON(A_DisplayData[v_Area]['blind-URL'],function(o_JSON){
			switch(o_JSON['switch']){
				case 'data':
					$('#display-header-text-'+v_Area).html(o_JSON['header']);
					A_DisplayData[v_Area]['data']=o_JSON['data'];
					if(F_ArrayKeyExists('list-ID',o_JSON)){A_DisplayData[v_Area]['list-ID']=o_JSON['list-ID'];}
					F_ManageData(v_Area,'base-list','display-'+v_Area,'list');
					//F_InsertData(v_Area,false,'base-list','display-'+v_Area,'list');
					o_Display.trigger('highlight-target');
					if(o_JSON['alias']=='materials'){F_RequestLists();}
					break;
				case '':default:
					break;
	...
	o_Display
		.trigger('show-navigation-icons')
		.trigger('request-blind-URL');

}

/* Array - Material Lists */
var A_MaterialLists=new Array();
/* Variable - Material Lists Start */
var V_MaterialListsStart=0;
/* Variable - Material Lists End */
var V_MaterialListsEnd=0;
/* Variable - Material Lists Current ID */
var V_MaterialListsCurrentID=0;
/* Variable - Material Lists Current Key */
var V_MaterialListsCurrentKey=0;
/* Variable - Material Lists Modifier */
var V_MaterialListsModifier=0;
...
/* Live Event - Clicked Line */
$('[name|="clicked-line"]').live('click',function(){
	var v_Target=$(this).attr('name').replace('clicked-line-','');
	var v_Area=$(this).parents('[id|="display"]').attr('id').replace('display-','');
	A_DisplayData[v_Area]['target']=v_Target;
	$('#display-'+v_Area).trigger('zoom-in');
});

var V_ListOnScreen=false;
$('.change-list-click2').live('click',function(){
	F_StartTest($(this).offset().left-16,$(this).offset().top-5);
	return;
	if(V_ListOnScreen){
		$('#new-screen').hide();
		$('#list-changer').hide();
		$('#list-changer-inside').html('');
		if(V_MaterialListsCurrentID!==A_DisplayData['materials']['list-ID']){
			$.getJSON('/quick/change-list/'+V_MaterialListsCurrentID,function(o_JSON){
				switch(o_JSON['switch']){
					case 'data':
						V_MaterialListsCurrentID=o_JSON['list-ID'];
						A_MaterialLists=o_JSON['data'];
		...
		$('#new-screen').fadeTo(0,0.65);
		$('#list-changer').css('left',($(this).position().left-16)).css('top',($(this).position().top+23)).show();
		$('#list-changer-inside').html(F_GetMaterialListInsideHTML());
		$('#list-changer-outside').html(F_GetMaterialListOutsideHTML());
	}
	V_ListOnScreen=!V_ListOnScreen;
});

/* Function - Get Material List Outside HTML */
function F_GetMaterialListOutsideHTML(){
	return '<font class="font-gold" style="font-size:13px;">Description</font><br/><div class="font-italic" style="font-size:12px;">'+A_MaterialLists[V_MaterialListsCurrentKey]['description']+'</div>';
}
/* Function - Get Material List Inside HTML */
function F_GetMaterialListInsideHTML(){
	var v_HTML='';
	var v_Start=0;
	var v_End=0;
	var v_Arrows=true;
	if(V_MaterialListsEnd>5){
		v_Start=0;
		v_End=v_Start+4;
	}else{
		v_End=V_MaterialListsEnd;
		v_Arrows=false;
	}
	v_HTML+='<div class="border-all-I-1-65 color-Y-2 corners-bottom-3 corners-top-3 image-lines" style="height:141px; left:5px; position:absolute; width:38px; z-index:59;"></div><div class="font-gold" style="font-size:13px; left:50px; position:absolute; z-index:60;">Name</div><div class="font-H" style="font-size:11px; left:12px; margin-top:2px; position:absolute; z-index:61;">'+(V_MaterialListsModifier+1)+' / '+V_MaterialListsEnd+'</div><div style="height:16px; position:relative; text-align:center; width:240px; z-index:61;"><img class="fake-link decrease-list" src="http://cdn1.lapcat.org/fugue/icons/control-090.png" style="'+((v_Arrows&&V_MaterialListsModifier>0)?'':'display:none; ')+'position:relative; height:16px; width:16px;" /></div>';
	v_HTML+='<div style="height:110px; text-align:center; overflow-y:hidden; vertical-align:top; width:320px;"><div id="list-changer-inside-adjustment" style="display:block; position:relative; top:0px; z-index:60;">';
	for(var v_Key in A_MaterialLists){
		//alert(V_MaterialListsCurrentID);
		v_HTML+='<div class="font-H" style="font-size:10px; float:left; height:18px; padding-top:3px; text-align:center; width:40px;">'+((A_MaterialLists[v_Key]['ID']==V_MaterialListsCurrentID)?'current':'')+'</div><div class="'+((A_MaterialLists[v_Key]['ID']==V_MaterialListsCurrentID)?'button-gold':'button-Y-35')+' shadow-or-light-Y-down change-list-key-click" id="change-list-key-'+v_Key+'" style="float:left; font-size:14px; height:18px; margin:1px; margin-left:2px; margin-right:2px; padding-left:3px; text-align:left; width:269px;">'+A_MaterialLists[v_Key]['name']+'</div>';
	}
	v_HTML+='</div></div>';
	v_HTML+='<div style="height:16px; text-align:center; width:240px;"><img class="fake-link increase-list" src="http://cdn1.lapcat.org/fugue/icons/control-270.png" style="'+((v_Arrows&&V_MaterialListsModifier<(V_MaterialListsEnd-1))?'':'display:none; ')+'height:16px; width:16px;" /></div>';
	return v_HTML;
}
/* Function - Change List HTML */
function F_ChangeListHTML(){
	V_MaterialListsCurrentKey=V_MaterialListsModifier;
	V_MaterialListsCurrentID=A_MaterialLists[V_MaterialListsModifier]['ID'];
	$('#list-changer-inside').html(F_GetMaterialListInsideHTML());
	$('#list-changer-outside').html(F_GetMaterialListOutsideHTML());
}
/* Live Event - Change List Key Click */
$('.change-list-key-click').live('click',function(){
	V_MaterialListsModifier=parseInt($(this).attr('id').replace('change-list-key-',''));
	F_ChangeListHTML();
});
/* Live Event - Decrease List (Key) */
$('.decrease-list').live('click',function(){
	$('.increase-list').show();
	V_MaterialListsModifier--;
	if(V_MaterialListsModifier<0){V_MaterialListsModifier=0;}
	F_ChangeListHTML();
	if(V_MaterialListsModifier>4){$('#list-changer-inside-adjustment').css('top',(-22*(V_MaterialListsModifier-4)));}
	if(V_MaterialListsModifier==0){$('.decrease-list').hide();}
});
/* Live Event - Increase List (Key) */
$('.increase-list').live('click',function(){
	$('.decrease-list').show();
	V_MaterialListsModifier++;
	if(V_MaterialListsModifier>V_MaterialListsEnd){V_MaterialListsModifier=V_MaterialListsEnd;}
	F_ChangeListHTML();
	if(V_MaterialListsModifier>4){$('#list-changer-inside-adjustment').css('top',(-22*(V_MaterialListsModifier-4)));}
	if(V_MaterialListsModifier>=V_MaterialListsEnd){$('.increase-list').hide();}
});
...
$('.menu-move-click2').live('click',function(){
	$('#static').replaceWith('');
...
var A_SendWithSearch={
	...
	'construct-3':{'similar':'text','specific':'text','date':'text','popular':'value','search':'value','tag':'value','type':'value'},
		...
		.bind('add-lines',function(){
			...
			for(var v_View in A_Cells[v_Construct]['views']){
				if(v_View=='month'){
					...
					if(A_Cells[v_Construct]['cover-images']){F_RequestImage(v_View,a_Data,'S',-1);}
				}
			}
		})
...
function F_CloseAccount(){
	...
	$('#anchored-message-box').css({bottom:-114});
}
/* Function - Close Help */
...
function F_HighlightMenu(){
	$('.menu-Z-35')
		.removeClass('menu-Z-35').addClass('button-gold');
	$('#menu-'+V_Area)
		.removeClass('menu-Y-65').addClass('button-gold');
}
/* Function - Line Click */
...
function F_OpenHelp(){
	...
	if(V_AnchoredMessageBoxOnScreen){
		...
		$('#anchored-message-box').css({bottom:12});
	}
}
/* Function - Open Help */
		...
		if(v_Pass){
			...
			a_Images[v_Key].bind('load',function(){
				if($(this).height()>1){
					...
					if(v_View=='list'||v_View=='large-image'){
						v_GoalWidth=36;
						v_GoalHeight=42;
...
$(document).ready(function(){
	...
	F_CloneConstruct('materials');
	return;

	F_AddElements(['black-screen','list-changer','list-options']);
	//F_AddMoreElements();
	F_ConstructStage();
	F_ConstructDisplay('events');
	F_ConstructDisplay('materials');
	F_ConstructDisplay('news');
	F_ConstructDisplay('databases');
	F_HighlightMenu();
	return;
