...
function F_ConstructStage(){
	...
	$('<div/>',{
		'id':'containers',
		'css':{'height':V_OpenLineMaximumHeight,'padding-left':16,'width':V_MaximumWidth}
	}).appendTo('body');
	...
	$('<div/>',{
		'id':'replace-one',
	  	'class':'button-blue-2 font-Y inside-border',
		'name':'normal',
		'css':{'height':40,'margin-bottom':3,'margin-top':3,'padding-left':3,'max-height':40,'width':'100%'}
...
function F_ManageData(v_Area,v_Element,v_Destination,v_Type){
	...
	switch(v_Type){
		case 'list':
			...
			for(var v_Counter=0;v_Counter<10;v_Counter++){
				if(!F_ArrayKeyExists(v_Counter,A_DisplayData[v_Area]['data'])||v_Counter>=v_Limit){
					$('#'+v_Destination+' #base-line-'+v_Counter).hide();
				}else{
...
function F_ConstructDisplay(v_Area){
	...
	$('<div/>',{
		...
		'css':{'display':'table','height':16,'width':'100%'}
	...
	if(v_Area=='materials'){
		...
		$('<div/>',{
			...
			'css':{'display':'table-cell','height':16,'padding-left':12,'padding-top':2,'width':26}
	...
	$('<div/>',{
		...
		'css':{'font-size':13,'height':16,'overflow':'hidden','padding-left':3,'width':'auto'}
	}).appendTo('#display-header-'+v_Area);
	...
	if(v_Area=='events'||v_Area=='materials'){
		...
		$('<div/>',{
			...
			'css':{'display':'none','height':16,'padding-left':12,'padding-top':2,'width':180}
		}).appendTo('#display-header-'+v_Area);

		$('<div/>',{
			'css':{'display':'table-cell','height':16,'width':3}
		}).appendTo('#display-header-'+v_Area);

	}
	
	/* Show Search (for Materials) */
	if(v_Area=='materials'){$('#display-header-search-'+v_Area).css('display','table-cell');}

	$('<div/>',{
		'class':'corners-bottom-3 corners-top-3 shadow-or-light-X-up',
		'id':'display-header-icons-'+v_Area,
		'css':{'display':'table-cell','height':16,'padding-right':3,'width':90}
	}).appendTo('#display-header-'+v_Area);
	...
	o_Display.bind('zoom-in',function(){
		...
		$('#display-container-'+v_Container).width('100%');
...
var V_QuarterMaximumWidth=Math.floor((V_MaximumWidth-6)*0.25);
