...
//
// Array - Hotkey Extra CSS
var A_HotkeyExtraCSS={
	'databases':'rgba(255,140,0,0.80)',
	'events':'',
	'materials':'',
	'other':'rgba(115,53,13,0.80)',
	'news':'',
	'hours':'',
	'hiring':''
};
...
//
// Function - Show Hotkey Positions
function F_ShowHotkeyPositions(){
	var v_HTML='<div>';
	for(var v_Key in A_Order){
		v_HTML+='<br/><br/>A_Order['+v_Key+']';
		v_HTML+='<br/>&nbsp;&nbsp;&nbsp;&nbsp;[area] = '+A_Order[v_Key]['area'];
		v_HTML+='<br/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;[id] = '+A_Order[v_Key]['id'];
		v_HTML+='<br/>&nbsp;&nbsp;&nbsp;&nbsp;[name] = '+A_Order[v_Key]['name'];
	}
	v_HTML+='</div>';
	$('#destination-content').append(v_HTML);
}
//
...
function F_AddHotkeyOLD(v_ID){
	...
	switch(a_Data['class']){
		case 'catalog':case 'log-in':
			v_Template=a_Data['class'];
			break;
		default:
			break;
	}
	...
	switch(a_Data['class']){
		case 'single':
			o_Hotkey.css('background-color',A_HotkeyExtraCSS[a_Data['area']]);
			break;
		default:
			break;
	}
	// Click Function - Expand
	$('#'+a_Data['name']).live('mouseenter',function(a_Event){$('#'+a_Data['name']+' #hotkey-options').css('height',18);});
	$('#'+a_Data['name']).live('mouseleave',function(a_Event){$('#'+a_Data['name']+' #hotkey-options').css('height',0);});
	$('#'+a_Data['name']).find('#hotkey-options-shift-right').live('click',function(a_Event){
		F_AddToLog('"'+a_Data['name']+'"-"shift right" in position '+a_Data['order']+' has been clicked.'); // ADD TO LOG
		var v_MyOrder=parseInt(a_Data['order']);
		switch(v_MyOrder){
			case A_Order.length-1:
				var v_OrderTarget=0;
				//alert('A1: '+v_OrderTarget);
				var v_Target='#destination-content';
				$(v_Target).prepend($('#'+a_Data['name']));
				break;
			default:
				var v_OrderTarget=v_MyOrder+1;
				//alert('B2: '+v_OrderTarget);
				var v_Target='#'+A_Order[v_OrderTarget]['name'];
				$(v_Target).after($('#'+a_Data['name']));
				break;
		}
		//
		var v_TargetHotkeyToTheRight='#'+A_Order[v_OrderTarget]['name'];
		//
		switch(v_MyOrder){
			case A_Order.length-1:
				var a_Holder=new Array();
				var a_LastOrder=A_Order[A_Order.length-1];
				a_Holder[0]=a_LastOrder;
				for(var v_Key in A_Order){if(v_Key<(A_Order.length-1)){a_Holder[a_Holder.length]=A_Order[v_Key];}}
				A_Order=a_Holder;
				for(var v_Key in A_Order){$('#'+A_Order[v_Key]['name']).trigger('change-order',v_Key);}
				//alert('A2: '+a_Data['order']);
				break;
			default:
				$(v_TargetHotkeyToTheRight).trigger('change-order',v_MyOrder);
				a_Data['order']=v_OrderTarget;
				//alert('B2: '+a_Data['order']);
				var a_OrderA=A_Order[v_OrderTarget];
				var a_OrderB=A_Order[v_MyOrder];
				A_Order[v_MyOrder]=a_OrderA;
				A_Order[v_OrderTarget]=a_OrderB;
				break;
		}
	});
	$('#'+a_Data['name']).find('#hotkey-options-shift-left').live('click',function(a_Event){
		F_AddToLog('"'+a_Data['name']+'"-"shift left" in position '+a_Data['order']+' has been clicked.'); // ADD TO LOG
		var v_MyOrder=parseInt(a_Data['order']);
		switch(v_MyOrder){
			case 0:
				var v_OrderTarget=A_Order.length-1;
				//alert('A: '+v_OrderTarget);
				var v_Target='#'+A_Order[v_OrderTarget]['name'];
				$(v_Target).after($('#'+a_Data['name']));
				break;
			case 1:
				var v_OrderTarget=0;
				//alert('B: '+v_OrderTarget);
				var v_Target='#destination-content';
				$(v_Target).prepend($('#'+a_Data['name']));
				break;
			default:
				var v_OrderTarget=v_MyOrder-1;
				//alert('C: '+v_OrderTarget);
				var v_Target='#'+A_Order[v_MyOrder-2]['name'];
				$(v_Target).after($('#'+a_Data['name']));
				break;
		}
		//
		var v_TargetHotkeyToTheLeft='#'+A_Order[v_OrderTarget]['name'];
		//
		switch(v_MyOrder){
			case 0:
				var a_Holder=new Array();
				var a_FirstOrder=A_Order[0];
				for(var v_Key in A_Order){if(v_Key>0){a_Holder[a_Holder.length]=A_Order[v_Key];}}
				A_Order=a_Holder;
				A_Order[A_Order.length]=a_FirstOrder;
				for(var v_Key in A_Order){$('#'+A_Order[v_Key]['name']).trigger('change-order',v_Key);}
				//alert('D: '+a_Data['order']);
				break;
			default:
				$(v_TargetHotkeyToTheLeft).trigger('change-order',v_MyOrder);
				a_Data['order']=v_OrderTarget;
				//alert('E: '+a_Data['order']);
				var a_OrderA=A_Order[v_OrderTarget];
				var a_OrderB=A_Order[v_MyOrder];
				A_Order[v_MyOrder]=a_OrderA;
				A_Order[v_OrderTarget]=a_OrderB;
	...
	$('#'+a_Data['name']).bind('change-order',function(a_Event,v_OrderTarget){
		a_Data['order']=v_OrderTarget;
		F_AddToLog('"'+a_Data['name']+'" has moved to position '+v_OrderTarget+'.'); // ADD TO LOG
	});
	
	//F_AddToLog('"'+a_Data['name']+'" has been constructed.'); // ADD TO LOG
	...
	$('#'+a_Data['name']).bind('request-records',function(){
		...
		//F_AddToLog('<font class="font-K">"'+a_Data['name']+'" has made it to gate II.</font>'); // ADD TO LOG
		$.getJSON('/quick/request/'+a_Data['id']+'/'+a_Data['stamp'],function(o_JSON){
			//F_AddToLog('<font class="font-K">"'+a_Data['name']+'" has made it to gate III.</font>'); // ADD TO LOG
	...
	$('#'+a_Data['name']).bind('check-display-status',function(){
		//F_AddToLog('<font class="font-K">"'+a_Data['name']+'" has made it to gate IV.</font>'); // ADD TO LOG
	...
	$('#'+a_Data['name']).bind('insert-data',function(){
		...
		switch(a_Data['class']){
			case 'catalog':case 'log-in':
				break;
			case 'list':case 'main-list':case 'single':
				var v_Limit=1;
				var v_Type='cell';
				switch(a_Data['class']){
					case 'single':v_Type=a_Data['class'];break;
					case 'main-list':v_Limit=2;break;
					case 'list':v_Limit=5;break;
					case '':default:break;
				}
				a_Data['cell-limit']=v_Limit;
				var o_Cell='';
				var v_ReplaceKey=0;
				var v_ReplaceType='';
				var v_Data='';
				var a_FirstKeys=F_GetFirstKeys(a_Data['records'],10);
				var v_NoData=true;
				for(var v_Key in a_FirstKeys){
					v_NoData=false;
					o_Cell=$('#'+v_Type+'-template').clone().attr('id',a_Data['name']+'-'+v_Type+'-'+v_Key);
					if(v_Key>=v_Limit){o_Cell.hide();}else{o_Cell.show();}
					o_Cell.find('[id|="replace"]').each(function(a_Event){
						...
						switch(v_ReplaceType){
							...
							case 'database-logo':$(this).attr('src','/lapcat/layout/databases/database-logo-'+v_Data+'.png');break;
							case 'link':
								if(V_InHouse||a_Data['class']=='single'){
									$(this).attr('href',v_Data);
								}else{
									$(this).attr('href',a_Data['records'][a_FirstKeys[v_Key]]['link_out']);
								}
								break;
							case '':default:
								break;
					...
					o_Cell.appendTo('#'+a_Data['name']+' #content');
				}
				if(v_NoData){$('#'+a_Data['name']+' #content').html('<font class="font-I" style="margin-left:4px; margin-top:12px;">There is no information to show.</font>').show();}
				break;
			default:
				break;
		}
	...
	// Function - Attempt To Hide Hotkey Options Remove
	$('#'+a_Data['name']).bind('attempt-to-hide-hotkey-options-remove',function(){if(V_LogStatus<0){$('#'+a_Data['name']).find('#hotkey-options-remove').hide();}});

	// Function - Start
	$('#'+a_Data['name']).bind('start',function(){
		switch(a_Data['class']){
			case 'list':case 'main-list':case 'single':
				//F_AddToLog('<font class="font-K">"'+a_Data['name']+'" has made it to gate I.</font>'); // ADD TO LOG
	...
	// Title Click
	$('#'+a_Data['name']).bind('title-click',function(){
		switch(a_Data['class']){
			case 'single':
				window.open($('#'+a_Data['name']+' #replace-link_in').attr('href'),'name');
				break;
			default:
				for(var v_Key in A_Hotkeys){$('#'+A_Hotkeys[v_Key]['area']+'-'+v_Key).hide();}
				$(this).trigger('expand',0);
				break;
		}
	});
	// Expand
	$('#'+a_Data['name']).bind('expand',function(v_Event,v_HighlightedCell){
		a_Parameters['highlighted-cell']=v_HighlightedCell;
		V_ExpandedTarget=a_Data['name'];
		$(this).find('#hotkey-options').hide();
		$(this).show();
		$('[id|="'+a_Data['name']+'-cell"]').show();
		$('.open-line').removeClass('open-line');
		$('#'+a_Data['name']+'-cell-'+v_HighlightedCell).addClass('open-line');
		$(this).css({'height':345,'width':945});
		$(this).find('#content-box').css({'height':495});
	});
	$('#'+a_Data['name']).bind('shrink',function(){
		V_ExpandedTarget=false;
		$(this).find('#hotkey-options').show();
		$(this).hide();
		$('[id|="'+a_Data['name']+'-cell"]').each(function(v_ID){if(v_ID<a_Data['cell-limit']){$(this).show();}else{$(this).hide();}});
		$('.open-line').removeClass('open-line');
		$(this).css({'height':195,'width':300});
		$(this).find('#content-box').css({'height':148});
	});

	//
	// S T A R T
	$('#'+a_Data['name']).trigger('start').trigger('attempt-to-hide-hotkey-options-remove');
}
//
// Live Event - Alert On Click
$('.clicked-on-title').live('click',function(){
	$('#'+$(this).parent().parent().attr('id')).trigger('title-click');
});
...
//
// Live Event - Expand On Click
$('.expand-on-click').live('click',function(){
	var A_Parts=$(this).parent().attr('id').split('-cell-');
	var v_ParentName=A_Parts[0];
	var v_CellID=A_Parts[1];
	var A_HotkeyInformation=v_ParentName.split('-');
	var v_HotkeyArea=A_HotkeyInformation[0];
	var v_HotkeyID=A_HotkeyInformation[1];
	//
	for(var v_Key in A_Hotkeys){$('#'+A_Hotkeys[v_Key]['area']+'-'+v_Key).hide();}
	$('#'+v_ParentName).trigger('expand',v_CellID);
});
//
// Live Event - Shrink All On Click
$('.shrink-all-on-click').live('click',function(){
	if(V_ExpandedTarget){$('#'+V_ExpandedTarget).trigger('shrink');}
	for(var v_Key in A_Hotkeys){$('#'+A_Hotkeys[v_Key]['area']+'-'+v_Key).show();
}});

// F U N C T I O N S
//
// Function - Clear Storage
function F_ClearStorage(){localStorage.clear();}
//
// Function - Shift Left
function F_ShiftLeft(v_OrderKey,v_HotkeyName){
	switch(v_OrderKey){
		case 0:
			var v_TargetOrderKey=A_Order.length-1;
			$('#'+A_Order[v_TargetOrderKey]['name']).after($('#'+v_HotkeyName));
			break;
		case 1:
			var v_TargetOrderKey=0;
			$('#destination-content').prepend($('#'+v_HotkeyName));
			break;
		default:
			var v_TargetOrderKey=v_OrderKey-1;
			$('#'+A_Order[v_OrderKey-2]['name']).after($('#'+v_HotkeyName));
			break;
	}
	switch(v_OrderKey){
		case 0:
			...
			break;
		default:
			var a_OrderA=A_Order[v_TargetOrderKey];
			var a_OrderB=A_Order[v_OrderKey];
			A_Order[v_OrderKey]=a_OrderA;
			A_Order[v_TargetOrderKey]=a_OrderB;
			break;
	}
	for(var v_Key in A_Order){$('#'+A_Order[v_Key]['name']).trigger('change-order',v_Key);}
}
...
function F_ShiftRight(v_OrderKey,v_HotkeyName){
	switch(v_OrderKey){
		case A_Order.length-1:
			var v_TargetOrderKey=0;
			$('#destination-content').prepend($('#'+v_HotkeyName));
...
function F_LogInOrCreateAccount(){
	$.getJSON('/lapcat/code/accounts.php',$('#account-log-in').serialize(),function(o_JSON){
		switch(o_JSON['switch']){
			case 'passed':
				V_LoggedIn=true;
				A_Hotkeys=o_JSON['hotkeys'];
				F_Start();
				//F_LogUser(true,true,o_JSON['theme']);
				break;
...
function F_Start(){
	...
	A_Order=new Array();
	for(var v_Key in A_Hotkeys){A_Order[A_Hotkeys[v_Key]['order_id']]={'area':A_Hotkeys[v_Key]['area'],'id':v_Key,'name':A_Hotkeys[v_Key]['area']+'-'+v_Key};}
	for(var v_Key in A_Order){F_AddHotkey(v_Key);}
}
// ************************************************************************************************************************************************************** //
// ************************************************************************************************************************************************************** //
// ************************************************************************************************************************************************************** //
//
// Function - Add Hotkey
function F_AddHotkey(v_OrderKey){
	var v_HotkeyID=A_Order[v_OrderKey]['id'];
	var a_Data={
		'area':A_Hotkeys[v_HotkeyID]['area'],
		'class':A_Hotkeys[v_HotkeyID]['class'],
		'cell-limit':A_Hotkeys[v_HotkeyID]['cell_limit'],
		'has-title-button':((parseInt(A_Hotkeys[v_HotkeyID]['has_title_button'])>0)?true:false),
		'id':v_HotkeyID,
		'name':A_Hotkeys[v_HotkeyID]['area']+'-'+v_HotkeyID,
		'template':A_Hotkeys[v_HotkeyID]['template'],
		'order':v_OrderKey,
		'title':A_Hotkeys[v_HotkeyID]['title'],
		'type':A_Hotkeys[v_HotkeyID]['type'],
		//
		'records':new Array(),
		'stamp':0,
		'total-records':0
	...
	// Clone Hotkey
	var o_Hotkey=$('#'+a_Data['template']+'-template').clone().attr('id',a_Data['name']).appendTo('#destination-content').show();
	// Attempt to turn this hotkey's title button off.
	if(!a_Data['has-title-button']){
		o_Hotkey.find('#title-button').hide();
	}else{
		o_Hotkey.find('#title-button').addClass(A_HotkeyButtons[a_Data['area']]);
		o_Hotkey.find('#title').html(a_Data['title']);
	}
	// Enable showing and hiding of the hotkey options box while hovered over this hotkey.
	$('#'+a_Data['name']).live('mouseenter',function(a_Event){$('#'+a_Data['name']+' #hotkey-options').css('height',18);});
	$('#'+a_Data['name']).live('mouseleave',function(a_Event){$('#'+a_Data['name']+' #hotkey-options').css('height',0);});
	$('#'+a_Data['name']).find('#hotkey-options-shift-left').live('click',function(a_Event){F_ShiftLeft(parseInt(a_Data['order']),a_Data['name']);});
	$('#'+a_Data['name']).find('#hotkey-options-shift-right').live('click',function(a_Event){F_ShiftRight(parseInt(a_Data['order']),a_Data['name']);});

	// F U N C T I O N S

	// Function - Change Order
	$('#'+a_Data['name']).bind('change-order',function(a_Event,v_OrderKey){a_Data['order']=v_OrderKey;});
	...
	$('#'+a_Data['name']).bind('check-display-status',function(){
		if(!a_Parameters['ready']){
			...
			// Need to compare to stored data to see if there is anything new to display. If so, then we need to change the hotkey contents. Otherwise we simply return.
			return;
		}
		$('#'+a_Data['name']).trigger('insert-data');
	});
	...
	$('#'+a_Data['name']).bind('expand',function(v_Event,v_HighlightedCell){
		...
		$('.open-line').removeClass('open-line');
		$('#'+a_Data['name']+'-cell-'+v_HighlightedCell).addClass('open-line');
		$(this).css({'height':515,'width':945});
		$(this).trigger('insert-expanded-data');
		$(this).find('#content-box').css({'height':495});
	});

	// Function - Insert Open Data
	$('#'+a_Data['name']).bind('insert-expanded-data',function(){
		var v_ReplaceKey=0;
		var v_ReplaceType='';
		var v_Data='';
		var o_OpenCell=$('#'+a_Data['area']+'-template').clone().attr('id',a_Data['name']+'-expanded-content');
		o_OpenCell.find('[id|="replace"]').each(function(a_Event){
			v_ReplaceKey=$(this).attr('id').replace('replace-','');
			v_ReplaceType=$(this).attr('name');
			v_Data=a_Data['records'][a_Parameters['highlighted-cell']][v_ReplaceKey];
			switch(v_ReplaceType){
				case 'normal':$(this).html(v_Data);break;
				case 'database-logo':$(this).attr('src','/lapcat/layout/databases/database-logo-'+v_Data+'.png');break;
				case 'library-image':$(this).attr('src','/lapcat/layout/library/150-100-'+(parseInt(v_Data)+1)+'.png');break;
				case 'link':$(this).attr('href',((V_InHouse||a_Data['type']=='single')?v_Data:a_Data['records'][a_Parameters['highlighted-cell']]['link_out']));break;
				case '':default:
					break;
			}
		});
		$(this).find('#expanded-content').html(o_OpenCell.show());
	});
	...
	$('#'+a_Data['name']).bind('insert-data',function(){
		switch(a_Data['type']){
			...
			default:
				var v_Type='cell';
				if(a_Data['type']=='single'){v_Type='single';}
				var o_Cell='';
				var v_ReplaceKey=0;
				var v_ReplaceType='';
				var v_Data='';
				...
				for(var v_Key in a_FirstKeys){
					v_NoData=false;
					o_Cell=$('#'+v_Type+'-template').clone().attr('id',a_Data['name']+'-'+v_Type+'-'+v_Key);
					if(v_Type=='single'){o_Cell.removeClass('button-Y-35').addClass(a_Data['class']);}
					if(v_Key>=a_Data['cell-limit']){o_Cell.hide();}else{o_Cell.show();}
					o_Cell.find('[id|="replace"]').each(function(a_Event){
						v_ReplaceKey=$(this).attr('id').replace('replace-','');
						v_ReplaceType=$(this).attr('name');
						v_Data=a_Data['records'][a_FirstKeys[v_Key]][v_ReplaceKey];
						switch(v_ReplaceType){
							case 'normal':$(this).html(v_Data);break;
							case 'database-logo':$(this).attr('src','/lapcat/layout/databases/database-logo-'+v_Data+'.png');break;
							case 'link':$(this).attr('href',((V_InHouse||a_Data['type']=='single')?v_Data:a_Data['records'][a_FirstKeys[v_Key]]['link_out']));break;
							case '':default:
								break;
						}
					});
...
$(document).ready(function(){
	$('body').append('<div class="border-all-K-1 font-K color-X-1 fake-link" onclick="javascript:F_ClearStorage();" style="position:relative; width:100px; height:40px; float:right; text-align:center; z-index:1000;">CLEAR STORAGE</div><div class="border-all-K-1 font-K color-X-1 fake-link" onclick="javascript:F_ShowLog();" style="position:relative; width:100px; height:40px; float:right; margin-top:2px; text-align:center; z-index:1000;">SHOW LOG</div><div class="border-all-K-1 font-K color-X-1 fake-link" onclick="javascript:F_ShowHotkeyPositions();" style="position:relative; width:100px; height:40px; float:right; margin-top:2px; text-align:center; z-index:1000;">DEBUG</div>');
	F_Start();
	if(!V_LoggedIn){$('#log-in-box').show();}
