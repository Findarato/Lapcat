...
(function(jQuery){
	 ...
	 */
 	jQuery.fn.dateter = function(options,custom_callback){
		...
		if(Settings.cellHeight<15){Settings.cellHeight=15;} //settings mins 
		if(Settings.cellWidth<12){Settings.cellWidth=12;} //settings mins 
		//this.attr("onBlur","javascript:setTimeout(\'jQuery.fn.hideBox(\"calBox\",300,\""+this.attr("id")+"\")\',200);");
		bodyHeight = jQuery('body').height();
		bodyWidth = jQuery('body').width();
		Shadow = $("<div style=\" z-index:50;\" id=\"dateterShadow\"/>").click(function(){
			jQuery("#calBox").hide();
			jQuery("#dateterShadow").hide();
			jQuery('#button-date-selector').css('z-index',6);
		})
		.css({display:"none",top:0,left:0,height:bodyHeight,width:bodyWidth,position:"absolute"})
		;
	...
	jQuery.fn.dateter.init = function(target){
		Display.monthsMoved = 0; //Fixes the issue where the month was getting confused	
		...
		jQuery(target).click(function(){jQuery("#calBox").show();jQuery("#dateterShadow").show();jQuery('#button-date-selector').css('z-index',-20);hideme=true;});
		jQuery('body').append(calBox = jQuery('<div class="OL-fred color-darker" id="calBox" style="z-index:50;padding:3px; height:'+Settings.height+';width:'+Settings.width+';" />').hide());
	...
	jQuery.fn.dateter.drawCalendar = function(calHolder,displaySettings,target){
		jQuery.dateter.display = displaySettings;
		...
		for(var a = 0; a < 6; a++){//Y
		//alert(Settings.cellHeight);
			calTable.append(jQuery('<tr id="w'+a+'" style="height:'+Settings.cellHeight+'px;"/>'));
			for (var b = 0; b < 7; b++) {//X
				jQuery("#w"+a).append(
					jQuery('<td class="font white" id="d'+cnt+'" style="width:'+Settings.cellWidth+'px;border-width:'+Settings.borderWidth+';border-style:'+Settings.borderStyle+';" />')
